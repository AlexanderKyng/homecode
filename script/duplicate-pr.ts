...
import { createOpencode } from "@openqcode-ai/sdk"
...
`)
  ...
  const openqcode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await openqcode.client.session.create()
    const result = await openqcode.client.session
      .prompt({
