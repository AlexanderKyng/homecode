...
import { createOpencode } from "@homecode-ai/sdk"
...
`)
  ...
  const homecode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await homecode.client.session.create()
    const result = await homecode.client.session
      .prompt({
