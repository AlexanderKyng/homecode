...
async function main() {
  ...
  // Delete any existing branches for homecode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating HomeCode extension to v${cleanVersion}"`
