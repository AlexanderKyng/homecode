...
const EXTENSION_NAME = "openqcode"
...
async function main() {
  ...
  // Delete any existing branches for openqcode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating OpenQCode extension to v${cleanVersion}"`
