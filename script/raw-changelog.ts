...
const repo = process.env.GH_REPO ?? "anomalyco/homecode"
const bot = ["actions-user", "github-actions[bot]", "homecode", "homecode-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/homecode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/homecode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/homecode/")) areas.add("core")
