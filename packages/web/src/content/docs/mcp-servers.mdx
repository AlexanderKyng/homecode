...
You can add external tools to OpenQCode using the _Model Context Protocol_, or MCP. OpenQCode supports both local and remote servers.
...
You can define MCP servers in your [OpenQCode Config](https://openqcode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/openqcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenQCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenQCode will:
...
If the server requires authentication, OpenQCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `openqcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenQCode will store the tokens securely in `~/.local/share/openqcode/mcp-auth.json`.
...
Your MCPs are available as tools in OpenQCode, alongside built-in tools. So you can manage them through the OpenQCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenQCode to your Sentry account.
