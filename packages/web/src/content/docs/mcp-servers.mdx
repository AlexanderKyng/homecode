...
You can add external tools to HomeCode using the _Model Context Protocol_, or MCP. HomeCode supports both local and remote servers.
...
You can define MCP servers in your [HomeCode Config](https://homecode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/homecode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
HomeCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, HomeCode will:
...
If the server requires authentication, HomeCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `homecode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, HomeCode will store the tokens securely in `~/.local/share/homecode/mcp-auth.json`.
...
Your MCPs are available as tools in HomeCode, alongside built-in tools. So you can manage them through the HomeCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect HomeCode to your Sentry account.
