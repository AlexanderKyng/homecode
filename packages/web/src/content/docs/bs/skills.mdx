...
Agent skills omogucavaju OpenQCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenQCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.openqcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/openqcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenQCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.openqcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.openqcode/skills/git-release/SKILL.md` ovako:
...
OpenQCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `openqcode.json`:
...
**Za ugradene agente** (u `openqcode.json`):
