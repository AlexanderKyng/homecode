...
Agent skills omogucavaju HomeCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
HomeCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.homecode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/homecode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, HomeCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.homecode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/homecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.homecode/skills/git-release/SKILL.md` ovako:
...
HomeCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `homecode.json`:
...
**Za ugradene agente** (u `homecode.json`):
