---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenQCode.
---

Dodaci vam omogućavaju da proširite OpenQCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenQCode.
...
- `.openqcode/plugins/` - Dodaci na nivou projekta
- `~/.config/openqcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openqcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openqcode/openqcode.json`)
2. Konfiguracija projekta (`openqcode.json`)
3. Globalni direktorij dodataka (`~/.config/openqcode/plugins/`)
4. Direktorij dodataka projekta (`.openqcode/plugins/`)
...
OpenQCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenQCode.
...
Spriječite openqcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openqcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji openqcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openqcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://openqcode.ai/docs/sdk) za detalje.
