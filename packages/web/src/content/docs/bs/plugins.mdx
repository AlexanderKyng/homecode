---
title: Dodaci
description: Napišite vlastite dodatke za proširenje HomeCode.
---

Dodaci vam omogućavaju da proširite HomeCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja HomeCode.
...
- `.homecode/plugins/` - Dodaci na nivou projekta
- `~/.config/homecode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/homecode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/homecode/homecode.json`)
2. Konfiguracija projekta (`homecode.json`)
3. Globalni direktorij dodataka (`~/.config/homecode/plugins/`)
4. Direktorij dodataka projekta (`.homecode/plugins/`)
...
HomeCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje HomeCode.
...
Spriječite homecode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u homecode:
...
Pomoćnik `tool` kreira prilagođeni alat koji homecode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za homecode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://homecode.ai/docs/sdk) za detalje.
