...
Postoje dvije vrste agenata u HomeCode: primarni agenti i podagenti.
...
HomeCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
HomeCode dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

HomeCode dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u HomeCode-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `homecode.json`:
...
- Globalno: `~/.config/homecode/agents/`
- Po projektu: `.homecode/agents/`
...
Ako temperatura nije navedena, HomeCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu HomeCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj HomeCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [HomeCode Zen](/docs/zen), koristili biste `homecode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `homecode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/homecode).
:::
