---
title: Server
description: Komunicirajte s openqcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openqcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openqcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openqcode.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openqcode`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openqcode serve` i za `openqcode web`.
...
Kada pokrenete `openqcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openqcode server za programsku interakciju sa openqcode-om.
:::

Ova arhitektura omogucava openqcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openqcode serve` da startate standalone server. Ako je openqcode TUI vec pokrenut, `openqcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenQCode [IDE](/docs/ide) pluginovi.
...
openqcode server izlaže sljedece API-je.
