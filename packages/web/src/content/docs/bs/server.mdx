---
title: Server
description: Komunicirajte s homecode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `homecode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji homecode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `homecode.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `homecode`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `homecode serve` i za `homecode web`.
...
Kada pokrenete `homecode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite homecode server za programsku interakciju sa homecode-om.
:::

Ova arhitektura omogucava homecode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `homecode serve` da startate standalone server. Ako je homecode TUI vec pokrenut, `homecode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste HomeCode [IDE](/docs/ide) pluginovi.
...
homecode server izlaže sljedece API-je.
