---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenQCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenQCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenQCode konfiguraciji.
...
u `~/.local/share/openqcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenQCode
config.
...
OpenQCode Zen je lista modela koje je obezbedio OpenQCode tim koji su bili
testirano i potvrđeno da dobro radi sa OpenQCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenQCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite openqcode i idite na [openqcode.ai/auth](https://openqcode.ai/auth).
...
Radi kao i svaki drugi provajder u OpenQCode i potpuno je opcionalan za korištenje.

---

## OpenQCode Go

OpenQCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenQCode tim i koji su testirani i verificirani da dobro rade s OpenQCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenQCode Go` i idite na [openqcode.ai/auth](https://openqcode.ai/zen).
...
Da biste koristili Amazon Bedrock s OpenQCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openqcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openqcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenQCode.
:::
...
Možete konfigurirati openqcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openqcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openqcode konfiguraciju.
...
da pohrani token u openqcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenQCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenQCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`openqcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `openqcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openqcode:
...
Za korištenje Google Vertex AI s OpenQCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openqcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openqcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [openqcode-helicone-session](https://github.com/H2Shami/openqcode-helicone-session) da automatski prijavite svaki OpenQCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenQCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati openqcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openqcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openqcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenQCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/openqcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenQCode:
...
3. Kopirajte API ključ za korištenje u OpenQCode.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenQCode, morate lokalno povući informacije o modelu:
...
OpenQCode Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenQCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenQCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenQCode Zen**.

```txt
   /connect
```

3. Unesite svoj OpenQCode API ključ.
...
Također možete dodati dodatne modele putem vaše openqcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openqcode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju openqcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openqcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openqcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenQCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openqcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenQCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openqcode konfiguraciji.
