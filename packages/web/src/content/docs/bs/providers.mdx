---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u HomeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

HomeCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj HomeCode konfiguraciji.
...
u `~/.local/share/homecode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem HomeCode
config.
...
HomeCode Zen je lista modela koje je obezbedio HomeCode tim koji su bili
testirano i potvrđeno da dobro radi sa HomeCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa HomeCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite homecode i idite na [homecode.ai/auth](https://homecode.ai/auth).
...
Radi kao i svaki drugi provajder u HomeCode i potpuno je opcionalan za korištenje.

---

## HomeCode Go

HomeCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža HomeCode tim i koji su testirani i verificirani da dobro rade s HomeCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `HomeCode Go` i idite na [homecode.ai/auth](https://homecode.ai/zen).
...
Da biste koristili Amazon Bedrock s HomeCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete homecode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `homecode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u HomeCode.
:::
...
Možete konfigurirati homecode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi homecode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju homecode konfiguraciju.
...
da pohrani token u homecode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
HomeCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate HomeCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`homecode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `homecode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s homecode:
...
Za korištenje Google Vertex AI s HomeCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće homecode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem homecodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [homecode-helicone-session](https://github.com/H2Shami/homecode-helicone-session) da automatski prijavite svaki HomeCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki HomeCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati homecode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati homecode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati homecode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za HomeCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/homecode) za detalje.
:::
...
Da biste koristili Ollama Cloud s HomeCode:
...
3. Kopirajte API ključ za korištenje u HomeCode.
...
6. **Važno**: Prije upotrebe modela oblaka u HomeCode, morate lokalno povući informacije o modelu:
...
HomeCode Zen je lista testiranih i verifikovanih modela koju je obezbedio HomeCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>HomeCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **HomeCode Zen**.

```txt
   /connect
```

3. Unesite svoj HomeCode API ključ.
...
Također možete dodati dodatne modele putem vaše homecode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše homecode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju homecode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s homecode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `homecode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju HomeCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `homecode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite HomeCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj homecode konfiguraciji.
