...
Da biste otklonili probleme s OpenQCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openqcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openqcode --log-level DEBUG`.

---

## Pohrana

OpenQCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/openqcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openqcode`
...
OpenQCode Desktop pokreće lokalni OpenQCode server (`openqcode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenQCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/openqcode/openqcode.jsonc` (ili `~/.config/openqcode/openqcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openqcode/openqcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openqcode\openqcode.jsonc`
...
OpenQCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/openqcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openqcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenQCode mogao ponovo izgraditi.

1. Potpuno zatvorite OpenQCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openqcode`
- **Linux**: obrišite `~/.cache/openqcode` (ili pokrenite `rm -rf ~/.cache/openqcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openqcode`

3. Ponovo pokrenite OpenQCode Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenQCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `openqcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, OpenQCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenQCode karakteristikama.

---

### Obavještenja se ne prikazuju

OpenQCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenQCode u postavkama vašeg OS-a, i
...
1. Zatvorite OpenQCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenQCode Desktop aplikacije):

- `openqcode.settings.dat` (URL zadanog servera za desktop)
- `openqcode.global.dat` i `openqcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s OpenQCode:
...
Da saznate kojim modelima imate pristup, pokrenite `openqcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openqcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenQCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openqcode`

2. Ponovo pokrenite OpenQCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenQCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenQCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
