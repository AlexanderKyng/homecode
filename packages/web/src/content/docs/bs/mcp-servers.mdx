...
Možete dodati vanjske alate u OpenQCode koristeći _Model Context Protocol_, ili MCP. OpenQCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenQCode Config](https://openqcode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/openqcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenQCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenQCode će:
...
Ako server zahtijeva autentifikaciju, OpenQCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `openqcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenQCode sigurno čuva tokene u `~/.local/share/openqcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenQCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenQCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenQCode sa vašim Sentry nalogom.
