...
Možete dodati vanjske alate u HomeCode koristeći _Model Context Protocol_, ili MCP. HomeCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [HomeCode Config](https://homecode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/homecode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
HomeCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, HomeCode će:
...
Ako server zahtijeva autentifikaciju, HomeCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `homecode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, HomeCode sigurno čuva tokene u `~/.local/share/homecode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u HomeCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz HomeCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete HomeCode sa vašim Sentry nalogom.
