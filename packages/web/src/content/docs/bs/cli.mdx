---
title: CLI
description: HomeCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

HomeCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
homecode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa HomeCode.
...
Pokrenite HomeCode terminalski korisnički interfejs.
...
HomeCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte HomeCode agentima.
...
Priključite terminal na već pokrenut HomeCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
homecode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim HomeCode backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `homecode`) |
...
HomeCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `homecode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

Kada se HomeCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u HomeCode.
...
Pokrenite HomeCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `homecode serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti HomeCode server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `homecode`) |
...
Pokrenite HomeCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
homecode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti HomeCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `homecode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše HomeCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili HomeCode dijeljenog URL-a.

```bash
homecode import <file>
```

Možete uvesti iz lokalne datoteke ili HomeCode dijeljenog URL-a.
...
Pokrenite HomeCode headless server sa web interfejsom.

```bash
homecode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup HomeCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `homecode`).
...
Deinstalirajte HomeCode i uklonite sve povezane datoteke.
...
Ažurira HomeCode na najnoviju verziju ili određenu verziju.
...
HomeCode CLI prihvata sljedeće globalne zastavice.
...
HomeCode se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `homecode`) |
