---
title: CLI
description: OpenQCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenQCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
openqcode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa OpenQCode.
...
Pokrenite OpenQCode terminalski korisnički interfejs.
...
OpenQCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte OpenQCode agentima.
...
Priključite terminal na već pokrenut OpenQCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
openqcode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim OpenQCode backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `openqcode`) |
...
OpenQCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `openqcode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

Kada se OpenQCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u OpenQCode.
...
Pokrenite OpenQCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `openqcode serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti OpenQCode server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `openqcode`) |
...
Pokrenite OpenQCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
openqcode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenQCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `openqcode`).
...
Upravljajte OpenQCode sesijama.
...
Prikaži statistiku upotrebe tokena i troškova za vaše OpenQCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili OpenQCode dijeljenog URL-a.

```bash
openqcode import <file>
```

Možete uvesti iz lokalne datoteke ili OpenQCode dijeljenog URL-a.
...
Pokrenite OpenQCode headless server sa web interfejsom.

```bash
openqcode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenQCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `openqcode`).
...
Deinstalirajte OpenQCode i uklonite sve povezane datoteke.
...
Ažurira OpenQCode na najnoviju verziju ili određenu verziju.
...
OpenQCode CLI prihvata sljedeće globalne zastavice.
...
OpenQCode se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `openqcode`) |
