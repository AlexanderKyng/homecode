...
U HomeCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, HomeCode koristi nasu `homecode` temu.
...
HomeCode dolazi sa vise ugradenih tema.
...
- Zele da HomeCode odgovara izgledu njihovog terminala
...
HomeCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/homecode/themes/*.json` ili `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.homecode/themes/*.json`
4. **Trenutni radni direktorij** - `./.homecode/themes/*.json`
