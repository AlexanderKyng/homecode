...
U OpenQCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenQCode koristi nasu `openqcode` temu.
...
OpenQCode dolazi sa vise ugradenih tema.
...
- Zele da OpenQCode odgovara izgledu njihovog terminala
...
OpenQCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/openqcode/themes/*.json` ili `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openqcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.openqcode/themes/*.json`
