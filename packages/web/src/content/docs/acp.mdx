---
title: ACP Support
description: Use OpenQCode in any ACP-compatible editor.
---

OpenQCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use OpenQCode via ACP, configure your editor to run the `openqcode acp` command.

The command starts OpenQCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'OpenQCode' agent in the AI Chat agent selector.
...
To use OpenQCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use OpenQCode as the ACP agent for chat.
...
OpenQCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your OpenQCode config
