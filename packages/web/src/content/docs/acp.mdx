---
title: ACP Support
description: Use HomeCode in any ACP-compatible editor.
---

HomeCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use HomeCode via ACP, configure your editor to run the `homecode acp` command.

The command starts HomeCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'HomeCode' agent in the AI Chat agent selector.
...
To use HomeCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use HomeCode as the ACP agent for chat.
...
HomeCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your HomeCode config
