...
HomeCode Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

GoはHomeCodeの他のプロバイダーと同様に機能します。HomeCode GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、HomeCodeを使用するために必須ではありません。
...
:::tip
HomeCodeでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
HomeCode Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

HomeCode Goは、HomeCodeの他のプロバイダーと同様に機能します。

1. **<a href={console}>HomeCode Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`HomeCode Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
HomeCode Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
HomeCode設定の[model id](/docs/config/#models)は、`homecode-go/<model-id>`という形式を使用します。たとえば、Kimi K2.6の場合は、設定で`homecode-go/kimi-k2.6`を使用します。
...
4. HomeCodeで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
