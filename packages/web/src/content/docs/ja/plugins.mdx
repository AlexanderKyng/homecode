---
title: プラグイン
description: OpenQCode を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで OpenQCode を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、OpenQCode のデフォルトの動作を変更したりできます。
...
- `.openqcode/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/openqcode/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/openqcode/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/openqcode/openqcode.json`)
2. プロジェクト設定 (`openqcode.json`)
3. グローバルプラグインディレクトリ (`~/.config/openqcode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.openqcode/plugins/`)
...
OpenQCode は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための OpenQCode SDK クライアント。
...
ここでは、OpenQCode を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://openqcode.ai/docs/sdk) を参照してください。
...
OpenQCode が `.env` ファイルを読み取らないようにします。
...
プラグインは OpenQCode にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、OpenQCode が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して OpenQCode で使用できます。
