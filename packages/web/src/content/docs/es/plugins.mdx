---
title: Complementos
description: Escriba sus propios complementos para extender OpenQCode.
---

Los complementos le permiten extender OpenQCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenQCode.
...
- `.openqcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/openqcode/plugins/` - Complementos globales
...
```json title="openqcode.json"
{
  ...
  "plugin": ["openqcode-helicone-session", "openqcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openqcode/node_modules/`.
...
1. Configuración global (`~/.config/openqcode/openqcode.json`)
2. Configuración del proyecto (`openqcode.json`)
3. Directorio global de complementos (`~/.config/openqcode/plugins/`)
4. Directorio de complementos del proyecto (`.openqcode/plugins/`)
...
```json title=".openqcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenQCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".openqcode/plugins/my-plugin.ts"
...
```js title=".openqcode/plugins/example.js"
...
- `client`: Un cliente SDK openqcode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openqcode.
...
```js title=".openqcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openqcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenQCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que openqcode lea archivos `.env`:

```javascript title=".openqcode/plugins/env-protection.js"
...
```javascript title=".openqcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a openqcode:

```ts title=".openqcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openqcode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que openqcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openqcode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://openqcode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".openqcode/plugins/compaction.ts"
import type { Plugin } from "@openqcode-ai/plugin"
...
```ts title=".openqcode/plugins/custom-compaction.ts"
import type { Plugin } from "@openqcode-ai/plugin"
