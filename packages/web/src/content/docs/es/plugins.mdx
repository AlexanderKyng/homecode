---
title: Complementos
description: Escriba sus propios complementos para extender HomeCode.
---

Los complementos le permiten extender HomeCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de HomeCode.
...
- `.homecode/plugins/` - Complementos a nivel de proyecto
- `~/.config/homecode/plugins/` - Complementos globales
...
```json title="homecode.json"
{
  ...
  "plugin": ["homecode-helicone-session", "homecode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/homecode/node_modules/`.
...
1. Configuración global (`~/.config/homecode/homecode.json`)
2. Configuración del proyecto (`homecode.json`)
3. Directorio global de complementos (`~/.config/homecode/plugins/`)
4. Directorio de complementos del proyecto (`.homecode/plugins/`)
...
```json title=".homecode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
HomeCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".homecode/plugins/my-plugin.ts"
...
```js title=".homecode/plugins/example.js"
...
- `client`: Un cliente SDK homecode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar homecode.
...
```js title=".homecode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "homecode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio HomeCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que homecode lea archivos `.env`:

```javascript title=".homecode/plugins/env-protection.js"
...
```javascript title=".homecode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a homecode:

```ts title=".homecode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@homecode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que homecode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para homecode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://homecode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".homecode/plugins/compaction.ts"
import type { Plugin } from "@homecode-ai/plugin"
...
```ts title=".homecode/plugins/custom-compaction.ts"
import type { Plugin } from "@homecode-ai/plugin"
