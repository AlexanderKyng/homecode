...
description: Cree herramientas que LLM pueda llamar en openqcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openqcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openqcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openqcode/tools/`.
...
```ts title=".openqcode/tools/database.ts" {1}
...
```ts title=".openqcode/tools/math.ts"
import { tool } from "@openqcode-ai/plugin"
...
```ts title=".openqcode/tools/bash.ts"
import { tool } from "@openqcode-ai/plugin"
...
```ts title=".openqcode/tools/project.ts" {8}
import { tool } from "@openqcode-ai/plugin"
...
```python title=".openqcode/tools/add.py"
import sys
...
```ts title=".openqcode/tools/python-add.ts" {10}
import { tool } from "@openqcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openqcode/tools/add.py")
