...
description: Cree herramientas que LLM pueda llamar en homecode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de homecode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.homecode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/homecode/tools/`.
...
```ts title=".homecode/tools/database.ts" {1}
...
```ts title=".homecode/tools/math.ts"
import { tool } from "@homecode-ai/plugin"
...
```ts title=".homecode/tools/bash.ts"
import { tool } from "@homecode-ai/plugin"
...
```ts title=".homecode/tools/project.ts" {8}
import { tool } from "@homecode-ai/plugin"
...
```python title=".homecode/tools/add.py"
import sys
...
```ts title=".homecode/tools/python-add.ts" {10}
import { tool } from "@homecode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".homecode/tools/add.py")
