---
title: Servidor
description: Interactuar con el servidor openqcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openqcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openqcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openqcode.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openqcode`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openqcode serve` como a `openqcode web`.
...
Cuando ejecuta `openqcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openqcode para interactuar con openqcode mediante programación.
:::

Esta arquitectura permite que openqcode admita múltiples clientes y le permite interactuar con openqcode mediante programación.

Puede ejecutar `openqcode serve` para iniciar un servidor independiente. Si tienes el
openqcode TUI ejecutándose, `openqcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenQCode [IDE](/docs/ide).
...
El servidor openqcode expone las siguientes API.
