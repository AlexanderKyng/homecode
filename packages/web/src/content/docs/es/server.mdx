---
title: Servidor
description: Interactuar con el servidor homecode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `homecode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente homecode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `homecode.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `homecode`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `homecode serve` como a `homecode web`.
...
Cuando ejecuta `homecode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor homecode para interactuar con homecode mediante programación.
:::

Esta arquitectura permite que homecode admita múltiples clientes y le permite interactuar con homecode mediante programación.

Puede ejecutar `homecode serve` para iniciar un servidor independiente. Si tienes el
homecode TUI ejecutándose, `homecode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos HomeCode [IDE](/docs/ide).
...
El servidor homecode expone las siguientes API.
