...
Para depurar problemas con HomeCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/homecode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\homecode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `homecode --log-level DEBUG`.

---

## Almacenamiento

homecode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/homecode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\homecode`
...
HomeCode Desktop ejecuta un servidor HomeCode local (el sidecar `homecode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `HomeCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/homecode/homecode.jsonc` (o `~/.config/homecode/homecode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/homecode/homecode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\homecode\homecode.jsonc`
...
HomeCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/homecode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\homecode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.homecode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que HomeCode pueda reconstruirlo.

1. Salga de HomeCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/homecode`
- **Linux**: eliminar `~/.cache/homecode` (o ejecutar `rm -rf ~/.cache/homecode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\homecode`

3. Reinicie el escritorio HomeCode.

---

### Solucionar problemas de conexión del servidor

HomeCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `homecode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio HomeCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de HomeCode.

---

### Notificaciones que no se muestran

HomeCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para HomeCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio HomeCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio HomeCode):

- `homecode.settings.dat` (URL del servidor predeterminado de escritorio)
- `homecode.global.dat` y `homecode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con HomeCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/homecode/issues**](https://github.com/anomalyco/homecode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**homecode.ai/discord**](https://homecode.ai/discord)
...
3. Asegúrese de tener la última versión con `homecode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `homecode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/homecode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\homecode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. homecode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/homecode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\homecode`

2. Reinicie homecode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a homecode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
homecode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
