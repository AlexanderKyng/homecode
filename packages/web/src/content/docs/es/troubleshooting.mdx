...
Para depurar problemas con OpenQCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openqcode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `openqcode --log-level DEBUG`.

---

## Almacenamiento

openqcode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/openqcode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openqcode`
...
OpenQCode Desktop ejecuta un servidor OpenQCode local (el sidecar `openqcode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `OpenQCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/openqcode/openqcode.jsonc` (o `~/.config/openqcode/openqcode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/openqcode/openqcode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openqcode\openqcode.jsonc`
...
OpenQCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/openqcode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openqcode\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que OpenQCode pueda reconstruirlo.

1. Salga de OpenQCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/openqcode`
- **Linux**: eliminar `~/.cache/openqcode` (o ejecutar `rm -rf ~/.cache/openqcode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\openqcode`

3. Reinicie el escritorio OpenQCode.

---

### Solucionar problemas de conexión del servidor

OpenQCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `openqcode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio OpenQCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de OpenQCode.

---

### Notificaciones que no se muestran

OpenQCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para OpenQCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio OpenQCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio OpenQCode):

- `openqcode.settings.dat` (URL del servidor predeterminado de escritorio)
- `openqcode.global.dat` y `openqcode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con OpenQCode:
...
3. Asegúrese de tener la última versión con `openqcode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `openqcode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\openqcode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. openqcode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\openqcode`

2. Reinicie openqcode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a openqcode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
openqcode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
