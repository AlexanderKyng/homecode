...
Con OpenQCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenQCode usa nuestro propio tema `openqcode`.
...
OpenQCode viene con varios temas integrados.
...
- Quiere que OpenQCode coincida con la apariencia de su terminal
...
OpenQCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/openqcode/themes/*.json` o `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.openqcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.openqcode/themes/*.json`
