...
Con HomeCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, HomeCode usa nuestro propio tema `homecode`.
...
HomeCode viene con varios temas integrados.
...
- Quiere que HomeCode coincida con la apariencia de su terminal
...
HomeCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/homecode/themes/*.json` o `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.homecode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.homecode/themes/*.json`
