---
title: CLI
description: Opciones y comandos de la CLI de OpenQCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de OpenQCode de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
openqcode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con OpenQCode mediante programación.
...
Inicie la interfaz de usuario del terminal OpenQCode.
...
La CLI de OpenQCode también tiene los siguientes comandos.

---

### agent

Administrar agentes para OpenQCode.
...
Conecte una terminal a un servidor backend OpenQCode que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
openqcode attach [url]
```

Esto permite usar TUI con un backend remoto OpenQCode. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `openqcode`) |
...
OpenQCode funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `openqcode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

Cuando se inicia OpenQCode, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en OpenQCode.
...
Ejecute openqcode en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `openqcode serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor openqcode en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `openqcode`) |
...
Inicie un servidor OpenQCode sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
openqcode serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad openqcode sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `openqcode`).
...
Administrar OpenQCode sesiones.
...
Enumere todas las sesiones OpenQCode.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones OpenQCode.
...
Importe datos de sesión desde un archivo JSON o una URL compartida OpenQCode.

```bash
openqcode import <file>
```

Puede importar desde un archivo local o una URL compartida OpenQCode.
...
Inicie un servidor OpenQCode sin cabeza con una interfaz web.

```bash
openqcode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a OpenQCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `openqcode`).
...
Desinstale OpenQCode y elimine todos los archivos relacionados.
...
Actualiza openqcode a la última versión o a una versión específica.
...
La CLI de OpenQCode toma las siguientes banderas globales.
...
OpenQCode se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `openqcode`) |
