---
title: CLI
description: Opciones y comandos de la CLI de HomeCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de HomeCode de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
homecode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con HomeCode mediante programación.
...
Inicie la interfaz de usuario del terminal HomeCode.
...
La CLI de HomeCode también tiene los siguientes comandos.

---

### agent

Administrar agentes para HomeCode.
...
Conecte una terminal a un servidor backend HomeCode que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
homecode attach [url]
```

Esto permite usar TUI con un backend remoto HomeCode. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `homecode`) |
...
HomeCode funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `homecode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

Cuando se inicia HomeCode, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en HomeCode.
...
Ejecute homecode en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `homecode serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor homecode en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `homecode`) |
...
Inicie un servidor HomeCode sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
homecode serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad homecode sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `homecode`).
...
Enumere todas las sesiones HomeCode.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones HomeCode.
...
Importe datos de sesión desde un archivo JSON o una URL compartida HomeCode.

```bash
homecode import <file>
```

Puede importar desde un archivo local o una URL compartida HomeCode.
...
Inicie un servidor HomeCode sin cabeza con una interfaz web.

```bash
homecode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a HomeCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `homecode`).
...
Desinstale HomeCode y elimine todos los archivos relacionados.
...
Actualiza homecode a la última versión o a una versión específica.
...
La CLI de HomeCode toma las siguientes banderas globales.
...
HomeCode se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `homecode`) |
