...
Las habilidades del agente permiten a HomeCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
HomeCode busca estas ubicaciones:

- Configuración del proyecto: `.homecode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/homecode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, HomeCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.homecode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/homecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.homecode/skills/git-release/SKILL.md` así:
...
HomeCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `homecode.json`:
...
**Para agentes integrados** (en `homecode.json`):
