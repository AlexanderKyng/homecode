...
Las habilidades del agente permiten a OpenQCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenQCode busca estas ubicaciones:

- Configuración del proyecto: `.openqcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/openqcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenQCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.openqcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.openqcode/skills/git-release/SKILL.md` así:
...
OpenQCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `openqcode.json`:
...
**Para agentes integrados** (en `openqcode.json`):
