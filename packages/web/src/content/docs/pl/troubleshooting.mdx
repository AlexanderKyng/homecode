...
Aby debugować problemy z openqcode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openqcode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `openqcode --log-level DEBUG`.

---

## Przechowywanie danych

openqcode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/openqcode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openqcode`
...
openqcode Desktop uruchamia w tle lokalny serwer openqcode (sidecar `openqcode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OpenQCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/openqcode/openqcode.jsonc` (lub `~/.config/openqcode/openqcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/openqcode/openqcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openqcode\openqcode.jsonc`
...
openqcode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/openqcode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openqcode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby openqcode mógł ją odbudować.

1. Całkowicie zamknij openqcode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/openqcode`
- **Linux**: usuń `~/.cache/openqcode` (lub uruchom `rm -rf ~/.cache/openqcode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\openqcode`

3. Uruchom ponownie openqcode Desktop.

---

### Napraw problemy z połączeniem z serwerem

openqcode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `openqcode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows openqcode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami openqcode.

---

### Brak powiadomień

openqcode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla openqcode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie openqcode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji openqcode Desktop):

- `openqcode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `openqcode.global.dat` i `openqcode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z openqcode:
...
3. Upewnij się, że masz najnowszą wersję z `openqcode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `openqcode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\openqcode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. openqcode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\openqcode`

2. Uruchom ponownie kod openqcode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to openqcode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
openqcode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
