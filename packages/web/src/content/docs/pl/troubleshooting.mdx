...
Aby debugować problemy z homecode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\homecode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `homecode --log-level DEBUG`.

---

## Przechowywanie danych

homecode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/homecode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\homecode`
...
homecode Desktop uruchamia w tle lokalny serwer homecode (sidecar `homecode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `HomeCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/homecode/homecode.jsonc` (lub `~/.config/homecode/homecode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/homecode/homecode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\homecode\homecode.jsonc`
...
homecode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/homecode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\homecode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby homecode mógł ją odbudować.

1. Całkowicie zamknij homecode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/homecode`
- **Linux**: usuń `~/.cache/homecode` (lub uruchom `rm -rf ~/.cache/homecode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\homecode`

3. Uruchom ponownie homecode Desktop.

---

### Napraw problemy z połączeniem z serwerem

homecode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `homecode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows homecode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami homecode.

---

### Brak powiadomień

homecode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla homecode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie homecode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji homecode Desktop):

- `homecode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `homecode.global.dat` i `homecode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z homecode:
...
3. Upewnij się, że masz najnowszą wersję z `homecode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `homecode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\homecode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. homecode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\homecode`

2. Uruchom ponownie kod homecode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to homecode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
homecode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
