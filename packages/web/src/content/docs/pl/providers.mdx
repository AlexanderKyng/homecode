---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w homecode.
---

import config from "../../../../config.mjs"
export const console = config.console

homecode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji homecode.
...
w `~/.local/share/homecode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim homecode
config.
...
HomeCode Zen to lista modeli dostarczonych przez zespół homecode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z homecode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od HomeCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `HomeCode Zen` i przejdź do [homecode.ai/auth](https://homecode.ai/zen).
...
Działa jak każdy inny dostawca w homecode i jest całkowicie opcjonalny w użyciu.

---

## HomeCode Go

HomeCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół homecode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z homecode.

1. Uruchom polecenie `/connect` w TUI, wybierz `HomeCode Go` i przejdź do [homecode.ai/auth](https://homecode.ai/zen).
...
Aby używać Amazon Bedrock z homecode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania homecode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `homecode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w homecode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować homecode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod homecode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji homecode.
...
homecode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować homecode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`homecode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem homecode:
...
Aby używać Google Vertex AI z homecode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania homecode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą homecode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [homecode-helicone-session](https://github.com/H2Shami/homecode-helicone-session), aby automatycznie rejestrować każdą konwersację homecode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja homecode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować homecode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować homecode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować homecode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem homecode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/homecode).
:::
...
Aby korzystać z Ollama Cloud z homecode:
...
3. Skopiuj klucz API do użycia w homecode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w homecode musisz pobrać informacje o modelu lokalnie:
...
HomeCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół homecode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>HomeCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **HomeCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API homecode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji homecode.
...
5. Możesz je także dostosować za pomocą konfiguracji homecode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z homecode:
...
Możesz także dostosować modele za pomocą konfiguracji homecode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z homecode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `homecode.json` w katalogu projektu:
...
Pola `limit` pozwalają homecode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `homecode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację homecode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji homecode.
