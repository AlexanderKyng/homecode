---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w openqcode.
---

import config from "../../../../config.mjs"
export const console = config.console

openqcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji openqcode.
...
w `~/.local/share/openqcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim openqcode
config.
...
OpenQCode Zen to lista modeli dostarczonych przez zespół openqcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z openqcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenQCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenQCode Zen` i przejdź do [openqcode.ai/auth](https://openqcode.ai/zen).
...
Działa jak każdy inny dostawca w openqcode i jest całkowicie opcjonalny w użyciu.

---

## OpenQCode Go

OpenQCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół openqcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z openqcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenQCode Go` i przejdź do [openqcode.ai/auth](https://openqcode.ai/zen).
...
Aby używać Amazon Bedrock z openqcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania openqcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `openqcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w openqcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować openqcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod openqcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji openqcode.
...
openqcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować openqcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`openqcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem openqcode:
...
Aby używać Google Vertex AI z openqcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania openqcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą openqcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [openqcode-helicone-session](https://github.com/H2Shami/openqcode-helicone-session), aby automatycznie rejestrować każdą konwersację openqcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja openqcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować openqcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować openqcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować openqcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem openqcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/openqcode).
:::
...
Aby korzystać z Ollama Cloud z openqcode:
...
3. Skopiuj klucz API do użycia w openqcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w openqcode musisz pobrać informacje o modelu lokalnie:
...
OpenQCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół openqcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenQCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenQCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API openqcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji openqcode.
...
5. Możesz je także dostosować za pomocą konfiguracji openqcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z openqcode:
...
Możesz także dostosować modele za pomocą konfiguracji openqcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z openqcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `openqcode.json` w katalogu projektu:
...
Pola `limit` pozwalają openqcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `openqcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację openqcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji openqcode.
