---
title: CLI
description: Opcje i polecenia wiersza poleceń HomeCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI HomeCode domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
homecode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać HomeCode programowo.
...
Uruchamia interfejs terminalowy (TUI) HomeCode.
...
Interfejs CLI HomeCode zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami HomeCode.
...
Dołącz terminal do już działającego serwera backendu HomeCode uruchomionego za pomocą `serve` lub `web`.

```bash
homecode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem HomeCode. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `homecode`) |
...
HomeCode integruje się z wieloma dostawcami. Możesz użyć `homecode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

Podczas uruchamiania HomeCode ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w HomeCode.
...
Uruchom HomeCode w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `homecode serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera HomeCode (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `homecode`) |
...
Uruchom serwer HomeCode (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
homecode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API HomeCode bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `homecode`).
...
Lista wszystkich sesji HomeCode.
...
Pokaż statystyki wykorzystania tokenów i sesji HomeCode.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania HomeCode.
...
Uruchom serwer HomeCode z interfejsem internetowym.

```bash
homecode web
```

Uruchamia to serwer HTTP i udostępnia HomeCode przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `homecode`).
...
Odinstaluj HomeCode i usuń wszystkie powiązane pliki.
...
Aktualizuj HomeCode do najnowszej lub określonej wersji.
...
Interfejs CLI HomeCode przyjmuje następujące flagi globalne dla każdego polecenia.
...
HomeCode można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `homecode`)    |
