---
title: CLI
description: Opcje i polecenia wiersza poleceń OpenQCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI OpenQCode domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
openqcode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać OpenQCode programowo.
...
Uruchamia interfejs terminalowy (TUI) OpenQCode.
...
Interfejs CLI OpenQCode zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami OpenQCode.
...
Dołącz terminal do już działającego serwera backendu OpenQCode uruchomionego za pomocą `serve` lub `web`.

```bash
openqcode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem OpenQCode. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `openqcode`) |
...
OpenQCode integruje się z wieloma dostawcami. Możesz użyć `openqcode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

Podczas uruchamiania OpenQCode ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w OpenQCode.
...
Uruchom OpenQCode w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `openqcode serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera OpenQCode (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `openqcode`) |
...
Uruchom serwer OpenQCode (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
openqcode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API OpenQCode bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `openqcode`).
...
Lista wszystkich sesji OpenQCode.
...
Pokaż statystyki wykorzystania tokenów i sesji OpenQCode.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania OpenQCode.
...
Uruchom serwer OpenQCode z interfejsem internetowym.

```bash
openqcode web
```

Uruchamia to serwer HTTP i udostępnia OpenQCode przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `openqcode`).
...
Odinstaluj OpenQCode i usuń wszystkie powiązane pliki.
...
Aktualizuj OpenQCode do najnowszej lub określonej wersji.
...
Interfejs CLI OpenQCode przyjmuje następujące flagi globalne dla każdego polecenia.
...
OpenQCode można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `openqcode`)    |
