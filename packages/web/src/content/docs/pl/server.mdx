---
title: Serwer
description: Interakcja z serwerem openqcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openqcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openqcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openqcode.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openqcode` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openqcode serve`, jak i `openqcode web`.
...
Kiedy uruchomisz `openqcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openqcode do programowej interakcji z openqcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez openqcode i programową interakcję z kodem otwartym.

Możesz uruchomić `openqcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
openqcode TUI działa, `openqcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openqcode [IDE](/docs/ide).
...
Serwer openqcode udostępnia następujące interfejsy API.
