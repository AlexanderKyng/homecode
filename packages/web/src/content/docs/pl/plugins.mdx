---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć homecode.
---

Wtyczki umożliwiające wyłączenie homecode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania homecode.
...
- `.homecode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/homecode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/homecode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/homecode/homecode.json`)
2. Konfiguracja projektu (`homecode.json`)
3. Globalny katalog wtyczek (`~/.config/homecode/plugins/`)
4. Katalog wtyczek projektu (`.homecode/plugins/`)
...
homecode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z homecode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do homecode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej homecode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj homecode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do homecode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod homecode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://homecode.ai/docs/sdk).
