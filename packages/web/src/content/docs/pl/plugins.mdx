---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openqcode.
---

Wtyczki umożliwiające wyłączenie openqcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openqcode.
...
- `.openqcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openqcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openqcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openqcode/openqcode.json`)
2. Konfiguracja projektu (`openqcode.json`)
3. Globalny katalog wtyczek (`~/.config/openqcode/plugins/`)
4. Katalog wtyczek projektu (`.openqcode/plugins/`)
...
openqcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openqcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openqcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openqcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openqcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openqcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openqcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://openqcode.ai/docs/sdk).
