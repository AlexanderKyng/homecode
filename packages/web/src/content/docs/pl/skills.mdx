...
Umiejętności agenta pozwalają homecode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
homecode przeszukuje te lokalizacje:

- Project config: `.homecode/skills/<name>/SKILL.md`
- Global config: `~/.config/homecode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu homecode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.homecode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/homecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.homecode/skills/git-release/SKILL.md` w ten sposób:
...
homecode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `homecode.json`:
...
**Dla agentów wbudowanych** (w `homecode.json`):
