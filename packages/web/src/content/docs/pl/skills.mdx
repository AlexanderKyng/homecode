...
Umiejętności agenta pozwalają openqcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openqcode przeszukuje te lokalizacje:

- Project config: `.openqcode/skills/<name>/SKILL.md`
- Global config: `~/.config/openqcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openqcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openqcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openqcode/skills/git-release/SKILL.md` w ten sposób:
...
openqcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openqcode.json`:
...
**Dla agentów wbudowanych** (w `openqcode.json`):
