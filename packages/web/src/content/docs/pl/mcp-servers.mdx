...
Możesz dodać dodatkowe informacje do homecode za pomocą _Model Context Protocol_, w skrócie MCP. homecode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [homecode Config](https://homecode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/homecode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
homecode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, homecode:
...
Jeśli serwer wymaga uwierzytelnienia, homecode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `homecode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji homecode bezpieczne przechowa tokeny w `~/.local/share/homecode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w homecode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie homecode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć homecode z kontem Sentry.
