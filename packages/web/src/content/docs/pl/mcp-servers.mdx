...
Możesz dodać dodatkowe informacje do openqcode za pomocą _Model Context Protocol_, w skrócie MCP. openqcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openqcode Config](https://openqcode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openqcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openqcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openqcode:
...
Jeśli serwer wymaga uwierzytelnienia, openqcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openqcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openqcode bezpieczne przechowa tokeny w `~/.local/share/openqcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openqcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openqcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openqcode z kontem Sentry.
