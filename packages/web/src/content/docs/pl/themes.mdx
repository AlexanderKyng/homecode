...
Dzięki homecode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie homecode używa naszego własnego motywu `homecode`.
...
homecode ma kilka wbudowanych motywów.
...
- Chcą, aby HomeCode pasował do wyglądu ich terminala
...
homecode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/homecode/themes/*.json` lub `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.homecode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.homecode/themes/*.json`
