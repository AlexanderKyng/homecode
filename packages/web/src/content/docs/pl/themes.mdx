...
Dzięki openqcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openqcode używa naszego własnego motywu `openqcode`.
...
openqcode ma kilka wbudowanych motywów.
...
- Chcą, aby OpenQCode pasował do wyglądu ich terminala
...
openqcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openqcode/themes/*.json` lub `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openqcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openqcode/themes/*.json`
