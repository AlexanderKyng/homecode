---
title: CLI
description: Opções e comandos da CLI do homecode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do homecode, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
homecode
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o homecode programaticamente.
...
Inicie a interface de usuário do terminal do homecode.
...
A CLI do homecode também possui os seguintes comandos.

---

### agent

Gerencie agentes para o homecode.
...
Anexe um terminal a um servidor backend do homecode já em execução, iniciado via comandos `serve` ou `web`.

```bash
homecode attach [url]
```

Isso permite usar o TUI com um backend homecode remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `homecode`) |
...
O homecode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `homecode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

Quando o homecode é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no homecode.
...
Execute o homecode em modo não interativo passando um prompt diretamente.
...
```bash "homecode run"
homecode run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `homecode serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
homecode run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor homecode em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `homecode`) |
...
Inicie um servidor homecode sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
homecode serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do homecode sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `homecode`).
...
Liste todas as sessões do homecode.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do homecode.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do homecode.

```bash
homecode import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do homecode.
...
Inicie um servidor homecode sem cabeça com uma interface web.

```bash
homecode web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o homecode através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `homecode`).
...
Desinstale o homecode e remova todos os arquivos relacionados.
...
Atualiza o homecode para a versão mais recente ou uma versão específica.
...
A CLI do homecode aceita as seguintes flags globais.
...
O homecode pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `homecode`) |
