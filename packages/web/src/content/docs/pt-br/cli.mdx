---
title: CLI
description: Opções e comandos da CLI do openqcode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do openqcode, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
openqcode
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o openqcode programaticamente.
...
Inicie a interface de usuário do terminal do openqcode.
...
A CLI do openqcode também possui os seguintes comandos.

---

### agent

Gerencie agentes para o openqcode.
...
Anexe um terminal a um servidor backend do openqcode já em execução, iniciado via comandos `serve` ou `web`.

```bash
openqcode attach [url]
```

Isso permite usar o TUI com um backend openqcode remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `openqcode`) |
...
O openqcode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `openqcode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

Quando o openqcode é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no openqcode.
...
Execute o openqcode em modo não interativo passando um prompt diretamente.
...
```bash "openqcode run"
openqcode run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `openqcode serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
openqcode run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor openqcode em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `openqcode`) |
...
Inicie um servidor openqcode sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
openqcode serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do openqcode sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `openqcode`).
...
Liste todas as sessões do openqcode.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do openqcode.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do openqcode.

```bash
openqcode import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do openqcode.
...
Inicie um servidor openqcode sem cabeça com uma interface web.

```bash
openqcode web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o openqcode através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `openqcode`).
...
Desinstale o openqcode e remova todos os arquivos relacionados.
...
Atualiza o openqcode para a versão mais recente ou uma versão específica.
...
A CLI do openqcode aceita as seguintes flags globais.
...
O openqcode pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `openqcode`) |
