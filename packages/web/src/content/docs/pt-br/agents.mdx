...
Existem dois tipos de agentes no openqcode; agentes primários e subagentes.
...
openqcode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
OpenQCode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

OpenQCode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do OpenQCode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `openqcode.json`:
...
- Global: `~/.config/openqcode/agents/`
- Por projeto: `.openqcode/agents/`

```markdown title="~/.config/openqcode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o openqcode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openqcode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openqcode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenQCode Zen](/docs/zen), você usaria `openqcode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="openqcode.json" {3-6,9-12}
{
  "$schema": "https://openqcode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="openqcode.json" {3-5,8-10}
{
  "$schema": "https://openqcode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `openqcode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/openqcode).
:::
...
```markdown title="~/.config/openqcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/openqcode/agents/security-auditor.md"
---
