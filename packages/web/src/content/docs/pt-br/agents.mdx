...
Existem dois tipos de agentes no homecode; agentes primários e subagentes.
...
homecode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
HomeCode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

HomeCode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do HomeCode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `homecode.json`:
...
- Global: `~/.config/homecode/agents/`
- Por projeto: `.homecode/agents/`

```markdown title="~/.config/homecode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o homecode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do homecode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do homecode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [HomeCode Zen](/docs/zen), você usaria `homecode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="homecode.json" {3-6,9-12}
{
  "$schema": "https://homecode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="homecode.json" {3-5,8-10}
{
  "$schema": "https://homecode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `homecode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/homecode).
:::
...
```markdown title="~/.config/homecode/agents/docs-writer.md"
---
...
```markdown title="~/.config/homecode/agents/security-auditor.md"
---
