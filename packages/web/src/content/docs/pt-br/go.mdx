...
O HomeCode Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no HomeCode. Você assina o HomeCode Go e
...
O HomeCode Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O HomeCode Go funciona como qualquer outro provedor no HomeCode.

1. Você entra no **<a href={console}>HomeCode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `HomeCode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o HomeCode Go.
:::
...
O HomeCode Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do HomeCode
usa o formato `homecode-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`homecode-go/kimi-k2.6` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o HomeCode também.
