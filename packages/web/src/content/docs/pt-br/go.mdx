...
O OpenQCode Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no OpenQCode. Você assina o OpenQCode Go e
...
O OpenQCode Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O OpenQCode Go funciona como qualquer outro provedor no OpenQCode.

1. Você entra no **<a href={console}>OpenQCode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `OpenQCode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o OpenQCode Go.
:::
...
O OpenQCode Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do OpenQCode
usa o formato `openqcode-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`openqcode-go/kimi-k2.6` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o OpenQCode também.
