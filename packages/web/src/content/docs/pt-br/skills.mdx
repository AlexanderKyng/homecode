...
As habilidades do agente permitem que o openqcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openqcode pesquisa nesses locais:

- Configuração do projeto: `.openqcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openqcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openqcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openqcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openqcode/skills/git-release/SKILL.md` assim:
...
O openqcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openqcode.json`:
...
**Para agentes embutidos** (em `openqcode.json`):
