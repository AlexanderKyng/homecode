...
As habilidades do agente permitem que o homecode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O homecode pesquisa nesses locais:

- Configuração do projeto: `.homecode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/homecode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o homecode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.homecode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/homecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.homecode/skills/git-release/SKILL.md` assim:
...
O homecode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `homecode.json`:
...
**Para agentes embutidos** (em `homecode.json`):
