...
Com o openqcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openqcode usa nosso próprio tema `openqcode`.
...
O openqcode vem com vários temas embutidos.
...
- Querem que o openqcode corresponda à aparência do seu terminal
...
O openqcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openqcode/themes/*.json` ou `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openqcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openqcode/themes/*.json`
