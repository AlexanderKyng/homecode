...
Com o homecode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o homecode usa nosso próprio tema `homecode`.
...
O homecode vem com vários temas embutidos.
...
- Querem que o homecode corresponda à aparência do seu terminal
...
O homecode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/homecode/themes/*.json` ou `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.homecode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.homecode/themes/*.json`
