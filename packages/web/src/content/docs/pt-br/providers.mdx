---
title: Provedores
description: Usando qualquer provedor LLM no openqcode.
---

import config from "../../../../config.mjs"
export const console = config.console

openqcode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do openqcode.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/openqcode/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do openqcode.
...
OpenQCode Zen é uma lista de modelos fornecidos pela equipe do openqcode que foram testados e verificados para funcionar bem com o openqcode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o OpenQCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `OpenQCode Zen` e acesse [openqcode.ai/zen](https://openqcode.ai/zen).
...
Funciona como qualquer outro provedor no openqcode e é completamente opcional.

---

## OpenQCode Go

OpenQCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do openqcode que foram
testados e verificados para funcionar bem com o openqcode.

1. Execute o comando `/connect` no TUI, selecione `OpenQCode Go` e acesse [openqcode.ai/zen](https://openqcode.ai/zen).
...
Para usar o Amazon Bedrock com o openqcode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o openqcode:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `openqcode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no openqcode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o openqcode para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o openqcode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do openqcode.
...
armazenar o token no armazenamento de autenticação do openqcode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
openqcode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o openqcode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `openqcode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `openqcode.json`:
...
Para usar sua assinatura do GitHub Copilot com o openqcode:
...
Para usar o Google Vertex AI com o openqcode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o openqcode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do openqcode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [openqcode-helicone-session](https://github.com/H2Shami/openqcode-helicone-session) para registrar automaticamente cada conversa do openqcode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do openqcode listada como uma sessão separada.
...
Você pode configurar o openqcode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o openqcode para usar modelos locais através do LM Studio.
...
Você pode configurar o openqcode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o openqcode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/openqcode) para detalhes.
:::
...
Para usar o Ollama Cloud com o openqcode:
...
3. Copie a chave da API para uso no openqcode.
...
6. **Importante**: Antes de usar modelos em nuvem no openqcode, você deve puxar as informações do modelo localmente:
...
OpenQCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do openqcode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>OpenQCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **OpenQCode Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do openqcode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do openqcode.
...
5. Você também pode personalizá-los através da sua configuração do openqcode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o openqcode:
...
Você também pode personalizar modelos através da sua configuração do openqcode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o openqcode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no openqcode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `openqcode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o openqcode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `openqcode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do openqcode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do openqcode.
