---
title: Plugins
description: Escreva seus próprios plugins para estender o openqcode.
---

Plugins permitem que você estenda o openqcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openqcode.
...
- `.openqcode/plugins/` - Plugins em nível de projeto
- `~/.config/openqcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openqcode/node_modules/`.
...
1. Configuração global (`~/.config/openqcode/openqcode.json`)
2. Configuração do projeto (`openqcode.json`)
3. Diretório de plugins global (`~/.config/openqcode/plugins/`)
4. Diretório de plugins do projeto (`.openqcode/plugins/`)
...
O openqcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do openqcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openqcode.
...
:::note
Se você estiver usando o aplicativo desktop openqcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openqcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao openqcode:
...
O helper `tool` cria uma ferramenta personalizada que o openqcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openqcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://openqcode.ai/docs/sdk) para detalhes.
