---
title: Plugins
description: Escreva seus próprios plugins para estender o homecode.
---

Plugins permitem que você estenda o homecode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do homecode.
...
- `.homecode/plugins/` - Plugins em nível de projeto
- `~/.config/homecode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/homecode/node_modules/`.
...
1. Configuração global (`~/.config/homecode/homecode.json`)
2. Configuração do projeto (`homecode.json`)
3. Diretório de plugins global (`~/.config/homecode/plugins/`)
4. Diretório de plugins do projeto (`.homecode/plugins/`)
...
O homecode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do homecode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o homecode.
...
:::note
Se você estiver usando o aplicativo desktop homecode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o homecode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao homecode:
...
O helper `tool` cria uma ferramenta personalizada que o homecode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o homecode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://homecode.ai/docs/sdk) para detalhes.
