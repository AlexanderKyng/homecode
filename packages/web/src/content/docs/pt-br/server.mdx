---
title: Servidor
description: Interaja com o servidor homecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `homecode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente homecode pode usar.
...
```bash
homecode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `homecode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
homecode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `homecode`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `homecode serve` quanto ao `homecode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password homecode serve
```
...
Quando você executa `homecode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor homecode para interagir com o homecode programaticamente.
:::

Essa arquitetura permite que o homecode suporte múltiplos clientes e permite que você interaja com o homecode programaticamente.

Você pode executar `homecode serve` para iniciar um servidor autônomo. Se você tiver o TUI do homecode em execução, `homecode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do homecode [IDE](/docs/ide).
...
O servidor homecode expõe as seguintes APIs.
