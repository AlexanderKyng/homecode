...
description: Crie ferramentas que o LLM pode chamar no homecode.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do homecode, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.homecode/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/homecode/tools/`.
