---
title: ACP Desteği
description: homecode'u ACP uyumlu herhangi bir editörde kullanın.
---

homecode, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
homecode'u ACP aracılığıyla kullanmak için düzenleyicinizi `homecode acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, homecode'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'HomeCode' ajanını kullanın.
...
homecode'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı homecode'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
homecode, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- homecode yapılandırmanızda yapılandırılmış MCP sunucuları
