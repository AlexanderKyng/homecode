...
openqcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openqcode kendi `openqcode` temasını kullanır.
...
openqcode birden fazla yerleşik temayla gelir.
...
- openqcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openqcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openqcode/themes/*.json` veya `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openqcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openqcode/themes/*.json`
