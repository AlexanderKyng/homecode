...
homecode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak homecode kendi `homecode` temasını kullanır.
...
homecode birden fazla yerleşik temayla gelir.
...
- homecode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
homecode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/homecode/themes/*.json` veya `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.homecode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.homecode/themes/*.json`
