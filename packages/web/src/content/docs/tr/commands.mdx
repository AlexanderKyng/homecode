...
`.openqcode/commands/test.md` oluştur:
...
openqcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

openqcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/openqcode/commands/`
- Proje başına: `.openqcode/commands/`
...
openqcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
