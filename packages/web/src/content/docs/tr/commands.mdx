...
`.homecode/commands/test.md` oluştur:
...
homecode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

homecode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/homecode/commands/`
- Proje başına: `.homecode/commands/`
...
homecode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
