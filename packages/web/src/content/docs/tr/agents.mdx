...
openqcode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
openqcode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
OpenQCode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

OpenQCode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `openqcode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse openqcode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global openqcode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
openqcode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [OpenQCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `openqcode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `openqcode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/openqcode).
:::
