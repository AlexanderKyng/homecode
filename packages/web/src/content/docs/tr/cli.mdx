---
title: CLI
description: homecode CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

homecode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
homecode
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, homecode ile programlı olarak etkileşim kurmanıza olanak tanır.
...
homecode terminal kullanıcı arayüzünü başlatır.
...
homecode CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

homecode için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir homecode arka uç sunucusuna bir terminal ekleyin.

```bash
homecode attach [url]
```

Bu, TUI öğesinin uzak bir homecode arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `homecode`) |
...
homecode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `homecode auth login` kullanabilirsiniz. Bu `~/.local/share/homecode/auth.json`'de saklanıyor.

```bash
homecode auth login
```

homecode başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları homecode'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek homecode'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `homecode serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir homecode sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `homecode`) |
...
API erişimi için headless bir homecode sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
homecode serve
```

Bu, TUI arayüzü olmadan homecode işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `homecode` şeklindedir).
...
Tüm homecode oturumlarını listeleyin.
...
homecode oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya homecode paylaşımından URL oturum verilerini içe aktarın.

```bash
homecode import <file>
```

Yerel bir dosyadan veya homecode paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir homecode sunucusu başlatın.

```bash
homecode web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla homecode'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `homecode` şeklindedir).
...
homecode'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
homecode'u en son sürüme veya belirli bir sürüme günceller.
...
homecode CLI aşağıdaki global bayrakları destekler.
...
homecode ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `homecode`) |
