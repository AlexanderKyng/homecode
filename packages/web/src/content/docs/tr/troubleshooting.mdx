...
homecode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\homecode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `homecode --log-level DEBUG`.

---

## Depolama

homecode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/homecode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\homecode` yapistirin
...
homecode Desktop arka planda yerel bir homecode sunucusu (`homecode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `homecode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/homecode/homecode.jsonc` (veya `~/.config/homecode/homecode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/homecode/homecode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\homecode\homecode.jsonc` yapistirin
...
homecode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/homecode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\homecode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), homecode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. homecode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/homecode`
- **Linux**: `~/.cache/homecode` dizinini silin (veya `rm -rf ~/.cache/homecode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\homecode` yapistirin

3. homecode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

homecode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`homecode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta homecode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, homecode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

homecode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda homecode icin bildirimler etkinse
...
1. homecode Desktop'u kapatin
2. Su dosyalari bulun ve silin (homecode Desktop uygulama veri dizininde yer alirlar):

- `homecode.settings.dat` (desktop varsayilan sunucu URL'si)
- `homecode.global.dat` ve `homecode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
homecode ile ilgili bir sorun yasiyorsaniz:
...
3. `homecode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `homecode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\homecode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. homecode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\homecode`

2. En guncel provider paketlerini yeniden kurmak icin homecode'u yeniden baslatin

Bu, homecode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
homecode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
