...
HomeCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, HomeCode'daki diğer sağlayıcılar gibi çalışır. HomeCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve HomeCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
HomeCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

HomeCode Go, HomeCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>HomeCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `HomeCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye HomeCode Go'ya abone olabilir.
:::
...
HomeCode Go aşağıdaki limitleri içerir:
...
HomeCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `homecode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `homecode-go/kimi-k2.6` kullanmalısınız.
...
HomeCode Go'yu şu amaçlarla oluşturduk:
...
4. HomeCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
