...
OpenQCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, OpenQCode'daki diğer sağlayıcılar gibi çalışır. OpenQCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenQCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenQCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OpenQCode Go, OpenQCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenQCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenQCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OpenQCode Go'ya abone olabilir.
:::
...
OpenQCode Go aşağıdaki limitleri içerir:
...
OpenQCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `openqcode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `openqcode-go/kimi-k2.6` kullanmalısınız.
...
OpenQCode Go'yu şu amaçlarla oluşturduk:
...
4. OpenQCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
