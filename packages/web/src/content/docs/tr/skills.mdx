...
Ajan becerileri, openqcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
openqcode su konumlari tarar:

- Proje konfigurasyonu: `.openqcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/openqcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda openqcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.openqcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.openqcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
openqcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`openqcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`openqcode.json` icinde):
