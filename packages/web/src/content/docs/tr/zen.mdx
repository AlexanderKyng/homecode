---
title: Zen
description: homecode tarafından sağlanan seçilmiş modeller listesi.
---
...
HomeCode Zen, HomeCode ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

Zen, HomeCode'daki diğer sağlayıcılar gibi çalışır. HomeCode Zen'de oturum açıp API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve HomeCode kullanmak için buna ihtiyacınız yoktur.
...
:::tip
HomeCode ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
HomeCode Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

HomeCode Zen, HomeCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>HomeCode Zen</a>**'de oturum açın, faturalandırma bilgilerinizi ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, HomeCode Zen'i seçin ve API anahtarınızı yapıştırın.
...
HomeCode yapılandırmanızdaki [model id](/docs/config/#models) `homecode/<model-id>` biçimini kullanır. Örneğin, GPT 5.5 için yapılandırmanızda `homecode/gpt-5.5` kullanırsınız.
...
- DeepSeek V4 Flash Free, sınırlı bir süre için HomeCode'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Nemotron 3 Super Free, sınırlı bir süre için HomeCode'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için HomeCode'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
HomeCode Zen'i şunlar için oluşturduk:
...
4. Başka herhangi bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenme olmamasını** sağlamak. Ayrıca HomeCode ile her zaman başka herhangi bir sağlayıcıyı da kullanabilmenizi sağlamak.
