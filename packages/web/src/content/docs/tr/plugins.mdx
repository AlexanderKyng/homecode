---
title: Eklentiler
description: HomeCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek HomeCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya HomeCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.homecode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/homecode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/homecode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/homecode/homecode.json`)
2. Project config (`homecode.json`)
3. Global eklenti dizini (`~/.config/homecode/plugins/`)
4. Proje eklenti dizini (`.homecode/plugins/`)
...
HomeCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir HomeCode SDK istemcisi.
...
HomeCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
HomeCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
HomeCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca HomeCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, HomeCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra HomeCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://homecode.ai/docs/sdk) bakın.
