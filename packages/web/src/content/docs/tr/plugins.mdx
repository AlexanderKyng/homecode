---
title: Eklentiler
description: OpenQCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenQCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenQCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.openqcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openqcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openqcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/openqcode/openqcode.json`)
2. Project config (`openqcode.json`)
3. Global eklenti dizini (`~/.config/openqcode/plugins/`)
4. Proje eklenti dizini (`.openqcode/plugins/`)
...
OpenQCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenQCode SDK istemcisi.
...
OpenQCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenQCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenQCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenQCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenQCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenQCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://openqcode.ai/docs/sdk) bakın.
