...
_Model Bağlam Protokolü_ veya MCP kullanarak homecode'a harici araçlar ekleyebilirsiniz. homecode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [homecode Config](https://homecode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/homecode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
homecode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde homecode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa homecode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `homecode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından homecode, belirteçleri `~/.local/share/homecode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra homecode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi homecode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve homecode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
