...
_Model Bağlam Protokolü_ veya MCP kullanarak openqcode'a harici araçlar ekleyebilirsiniz. openqcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openqcode Config](https://openqcode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/openqcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openqcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openqcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openqcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openqcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openqcode, belirteçleri `~/.local/share/openqcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra openqcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openqcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve openqcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
