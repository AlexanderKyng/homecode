---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenQCode zu erweitern.
---

Mit Plugins können Sie OpenQCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenQCode zu ändern.
...
- `.openqcode/plugins/` – Plugins auf Projektebene
- `~/.config/openqcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openqcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openqcode/openqcode.json`)
2. Projektkonfiguration (`openqcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openqcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openqcode/plugins/`)
...
OpenQCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenQCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenQCode erweitern können.
...
:::note
Wenn Sie die OpenQCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenQCode `.env`-Dateien liest:
...
Plugins können OpenQCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenQCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenQCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://openqcode.ai/docs/sdk).
