---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um HomeCode zu erweitern.
---

Mit Plugins können Sie HomeCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von HomeCode zu ändern.
...
- `.homecode/plugins/` – Plugins auf Projektebene
- `~/.config/homecode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/homecode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/homecode/homecode.json`)
2. Projektkonfiguration (`homecode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/homecode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.homecode/plugins/`)
...
HomeCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein HomeCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie HomeCode erweitern können.
...
:::note
Wenn Sie die HomeCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass HomeCode `.env`-Dateien liest:
...
Plugins können HomeCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das HomeCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für HomeCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://homecode.ai/docs/sdk).
