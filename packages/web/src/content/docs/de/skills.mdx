...
Agent Skills erlauben HomeCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
HomeCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft HomeCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.homecode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/homecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.homecode/skills/git-release/SKILL.md` zum Beispiel so:
...
HomeCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `homecode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `homecode.json`):
