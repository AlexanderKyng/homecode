...
Agent Skills erlauben OpenQCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenQCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenQCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.openqcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.openqcode/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenQCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `openqcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `openqcode.json`):
