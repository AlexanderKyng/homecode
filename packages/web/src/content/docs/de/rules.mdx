---
title: Regeln
description: Lege eigene Anweisungen fuer openqcode fest.
---

Du kannst openqcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openqcode den Befehl `/init` aus.
...
So kann openqcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openqcode sie zu erweitern.
...
openqcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openqcode/AGENTS.md` speichern.
Diese gelten in allen openqcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenQCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openqcode/AGENTS.md` existiert)
...
Beim Start sucht openqcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openqcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openqcode.json` oder in `~/.config/openqcode/openqcode.json` angeben.
...
openqcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openqcode.json

Empfohlen ist das `instructions`-Feld in der `openqcode.json`:
...
Du kannst openqcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openqcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
