---
title: Regeln
description: Lege eigene Anweisungen fuer homecode fest.
---

Du kannst homecode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in homecode den Befehl `/init` aus.
...
So kann homecode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht homecode sie zu erweitern.
...
homecode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/homecode/AGENTS.md` speichern.
Diese gelten in allen homecode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt HomeCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/homecode/AGENTS.md` existiert)
...
Beim Start sucht homecode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/homecode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `homecode.json` oder in `~/.config/homecode/homecode.json` angeben.
...
homecode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit homecode.json

Empfohlen ist das `instructions`-Feld in der `homecode.json`:
...
Du kannst homecode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `homecode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
