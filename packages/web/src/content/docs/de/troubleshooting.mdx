...
Wenn OpenQCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openqcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `openqcode --log-level DEBUG`.

---

## Speicher

openqcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/openqcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openqcode`
...
OpenQCode Desktop startet im Hintergrund einen lokalen OpenQCode-Server (`openqcode-cli`-Sidecar).
...
- Nur macOS: `OpenQCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/openqcode/openqcode.jsonc` (oder `~/.config/openqcode/openqcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/openqcode/openqcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openqcode\openqcode.jsonc`
...
OpenQCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/openqcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openqcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OpenQCode ihn neu aufbauen kann.

1. Beende OpenQCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/openqcode`
- **Linux**: loesche `~/.cache/openqcode` (oder fuehre aus: `rm -rf ~/.cache/openqcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\openqcode`

3. Starte OpenQCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

OpenQCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `openqcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt OpenQCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OpenQCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OpenQCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OpenQCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende OpenQCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OpenQCode Desktop):

- `openqcode.settings.dat` (desktop default server URL)
- `openqcode.global.dat` und `openqcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit OpenQCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `openqcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\openqcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. openqcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\openqcode`

2. Starte openqcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt openqcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
openqcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
