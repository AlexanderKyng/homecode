...
Wenn HomeCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\homecode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `homecode --log-level DEBUG`.

---

## Speicher

homecode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/homecode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\homecode`
...
HomeCode Desktop startet im Hintergrund einen lokalen HomeCode-Server (`homecode-cli`-Sidecar).
...
- Nur macOS: `HomeCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/homecode/homecode.jsonc` (oder `~/.config/homecode/homecode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/homecode/homecode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\homecode\homecode.jsonc`
...
HomeCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/homecode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\homecode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit HomeCode ihn neu aufbauen kann.

1. Beende HomeCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/homecode`
- **Linux**: loesche `~/.cache/homecode` (oder fuehre aus: `rm -rf ~/.cache/homecode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\homecode`

3. Starte HomeCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

HomeCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `homecode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt HomeCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit HomeCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

HomeCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer HomeCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende HomeCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von HomeCode Desktop):

- `homecode.settings.dat` (desktop default server URL)
- `homecode.global.dat` und `homecode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit HomeCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `homecode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\homecode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. homecode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\homecode`

2. Starte homecode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt homecode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
homecode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
