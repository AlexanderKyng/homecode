...
HomeCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in HomeCode. Du abonnierst HomeCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
HomeCode zu verwenden.
...
HomeCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

HomeCode Go funktioniert wie jeder andere Provider in HomeCode.

1. Du meldest dich bei **<a href={console}>HomeCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `HomeCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann HomeCode Go abonnieren.
:::
...
HomeCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner HomeCode Config verwendet das Format `homecode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `homecode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben HomeCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit HomeCode zu nutzen.
