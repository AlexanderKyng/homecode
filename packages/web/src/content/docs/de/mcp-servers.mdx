...
Sie können externe Tools zu HomeCode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. HomeCode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [HomeCode Config](https://homecode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/homecode`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
HomeCode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird HomeCode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von HomeCode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `homecode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert HomeCode die Token sicher in `~/.local/share/homecode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in HomeCode verfügbar. Sie können sie also wie jedes andere Tool über die HomeCode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und HomeCode mit Ihrem Sentry-Konto zu verbinden.
