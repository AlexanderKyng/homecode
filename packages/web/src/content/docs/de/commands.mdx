...
Erstellen Sie `.openqcode/commands/test.md`:

```md title=".openqcode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die OpenQCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem OpenQCode [config](/docs/config):

```json title="openqcode.jsonc" {4-12}
{
...
- Global: `~/.config/openqcode/commands/`
- Pro Projekt: `.openqcode/commands/`

```markdown title="~/.config/openqcode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".openqcode/commands/component.md"
---
...
```md title=".openqcode/commands/create-file.md"
---
...
```md title=".openqcode/commands/analyze-coverage.md"
---
...
```md title=".openqcode/commands/review-changes.md"
---
...
```md title=".openqcode/commands/review-component.md"
---
...
OpenQCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
