---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in HomeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

HomeCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer HomeCode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem HomeCode anpassen
config.
...
HomeCode Zen ist eine Liste von Modellen, die vom HomeCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit HomeCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit HomeCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie HomeCode aus und gehen Sie zu [homecode.ai/auth](https://homecode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in HomeCode und ist völlig optional.

---

## HomeCode Go

HomeCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom HomeCode-Team getestet und verifiziert wurden, dass sie gut mit HomeCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `HomeCode Go` und gehen Sie zu [homecode.ai/auth](https://homecode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit HomeCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie HomeCode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `homecode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in HomeCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können homecode so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit HomeCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre HomeCode-Konfiguration hinzufügen.
...
um das Token im HomeCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
HomeCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von HomeCode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`homecode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit HomeCode:
...
So verwenden Sie Google Vertex AI mit HomeCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie HomeCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über HomeCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [homecode-helicone-session](https://github.com/H2Shami/homecode-helicone-session)-Plugin, um jede HomeCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede HomeCode-Konversation als separate Sitzung aufgeführt.
...
Sie können HomeCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können HomeCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können HomeCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für HomeCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/homecode).
:::
...
So verwenden Sie Ollama Cloud mit HomeCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in HomeCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in HomeCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
HomeCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom HomeCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>HomeCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **HomeCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren HomeCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre HomeCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre HomeCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit HomeCode:
...
Sie können Modelle auch über Ihre HomeCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit HomeCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `homecode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann HomeCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `homecode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die HomeCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer HomeCode-Konfiguration übereinstimmt.
