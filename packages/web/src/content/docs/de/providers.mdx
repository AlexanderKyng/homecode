---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OpenQCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenQCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OpenQCode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem OpenQCode anpassen
config.
...
OpenQCode Zen ist eine Liste von Modellen, die vom OpenQCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OpenQCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenQCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenQCode aus und gehen Sie zu [openqcode.ai/auth](https://openqcode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in OpenQCode und ist völlig optional.

---

## OpenQCode Go

OpenQCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OpenQCode-Team getestet und verifiziert wurden, dass sie gut mit OpenQCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenQCode Go` und gehen Sie zu [openqcode.ai/auth](https://openqcode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit OpenQCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OpenQCode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `openqcode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OpenQCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können openqcode so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OpenQCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OpenQCode-Konfiguration hinzufügen.
...
um das Token im OpenQCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OpenQCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OpenQCode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`openqcode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OpenQCode:
...
So verwenden Sie Google Vertex AI mit OpenQCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OpenQCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OpenQCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [openqcode-helicone-session](https://github.com/H2Shami/openqcode-helicone-session)-Plugin, um jede OpenQCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OpenQCode-Konversation als separate Sitzung aufgeführt.
...
Sie können OpenQCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können OpenQCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können OpenQCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OpenQCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/openqcode).
:::
...
So verwenden Sie Ollama Cloud mit OpenQCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OpenQCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OpenQCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenQCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenQCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OpenQCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OpenQCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OpenQCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OpenQCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OpenQCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OpenQCode:
...
Sie können Modelle auch über Ihre OpenQCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OpenQCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `openqcode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann OpenQCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `openqcode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OpenQCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OpenQCode-Konfiguration übereinstimmt.
