...
Mit HomeCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet HomeCode das `homecode`-Theme.
...
HomeCode liefert mehrere eingebaute Themes mit.
...
- HomeCode optisch ans Terminal anpassen wollen
...
HomeCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/homecode/themes/*.json` oder `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.homecode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.homecode/themes/*.json`
