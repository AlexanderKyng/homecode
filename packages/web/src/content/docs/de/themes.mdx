...
Mit OpenQCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenQCode das `openqcode`-Theme.
...
OpenQCode liefert mehrere eingebaute Themes mit.
...
- OpenQCode optisch ans Terminal anpassen wollen
...
OpenQCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/openqcode/themes/*.json` oder `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.openqcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.openqcode/themes/*.json`
