---
title: Server
description: Nutze den homecode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `homecode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein homecode-Client nutzen kann.
...
Der Benutzername ist standardmaessig `homecode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `homecode serve` und `homecode web`.
...
Wenn du `homecode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den homecode-Server, um homecode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `homecode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `homecode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die HomeCode-[IDE](/docs/ide)-Plugins.
...
Der homecode-Server stellt folgende APIs bereit.
