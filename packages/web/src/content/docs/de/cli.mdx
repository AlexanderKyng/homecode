---
title: CLI
description: OpenQCode CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der OpenQCode CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
openqcode
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit OpenQCode interagieren.
...
Starten Sie die Terminalbenutzeroberfläche OpenQCode.
...
Der OpenQCode CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für OpenQCode verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden OpenQCode-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
openqcode attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend OpenQCode. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `openqcode`) |
...
OpenQCode wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `openqcode auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/openqcode/auth.json` gespeichert.

```bash
openqcode auth login
```

Wenn OpenQCode startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in OpenQCode sehen möchten.
...
Führen Sie OpenQCode im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `openqcode serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden OpenQCode-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `openqcode`)                  |
...
Starten Sie einen Headless-OpenQCode-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
openqcode serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf OpenQCode-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `openqcode`).
...
Listen Sie alle OpenQCode-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre OpenQCode-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer OpenQCode-Freigabe-URL.

```bash
openqcode import <file>
```

Sie können aus einer lokalen Datei oder einer OpenQCode-Freigabe-URL importieren.
...
Starten Sie einen Headless-OpenQCode-Server mit einer Weboberfläche.

```bash
openqcode web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf OpenQCode zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `openqcode`).
...
Deinstallieren Sie OpenQCode und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert OpenQCode auf die neueste Version oder eine bestimmte Version.
...
Der OpenQCode CLI akzeptiert die folgenden globalen Flags.
...
OpenQCode kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `openqcode`) |
