---
title: CLI
description: HomeCode CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der HomeCode CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
homecode
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit HomeCode interagieren.
...
Starten Sie die Terminalbenutzeroberfläche HomeCode.
...
Der HomeCode CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für HomeCode verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden HomeCode-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
homecode attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend HomeCode. Zum Beispiel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `homecode`) |
...
HomeCode wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `homecode auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/homecode/auth.json` gespeichert.

```bash
homecode auth login
```

Wenn HomeCode startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in HomeCode sehen möchten.
...
Führen Sie HomeCode im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `homecode serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden HomeCode-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `homecode`)                  |
...
Starten Sie einen Headless-HomeCode-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
homecode serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf HomeCode-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `homecode`).
...
Listen Sie alle HomeCode-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre HomeCode-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer HomeCode-Freigabe-URL.

```bash
homecode import <file>
```

Sie können aus einer lokalen Datei oder einer HomeCode-Freigabe-URL importieren.
...
Starten Sie einen Headless-HomeCode-Server mit einer Weboberfläche.

```bash
homecode web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf HomeCode zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `homecode`).
...
Deinstallieren Sie HomeCode und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert HomeCode auf die neueste Version oder eine bestimmte Version.
...
Der HomeCode CLI akzeptiert die folgenden globalen Flags.
...
HomeCode kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `homecode`) |
