...
There are two types of agents in HomeCode; primary agents and subagents.
...
HomeCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
HomeCode comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

HomeCode comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into HomeCode's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `homecode.json` config file:
...
If no temperature is specified, HomeCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global HomeCode config and the project specific config.
...
The model ID in your HomeCode config uses the format `provider/model-id`. For example, if you're using [HomeCode Zen](/docs/zen), you would use `homecode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `homecode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/homecode).
:::
