---
title: الخادم
description: تفاعل مع خادم openqcode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `openqcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل openqcode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `openqcode.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `openqcode`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `openqcode serve` و `openqcode web`.
...
عند تشغيل `openqcode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم openqcode للتفاعل مع openqcode برمجيا.
:::

تتيح هذه البنية لـ openqcode دعم عدة عملاء وتمكّنك من التفاعل مع openqcode برمجيا.

يمكنك تشغيل `openqcode serve` لبدء خادم مستقل. إذا كانت واجهة openqcode في terminal (TUI)
قيد التشغيل، فسيبدأ `openqcode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenQCode لـ [IDE](/docs/ide).
...
يعرض خادم openqcode واجهات API التالية.
