---
title: الخادم
description: تفاعل مع خادم homecode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `homecode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل homecode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `homecode.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `homecode`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `homecode serve` و `homecode web`.
...
عند تشغيل `homecode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم homecode للتفاعل مع homecode برمجيا.
:::

تتيح هذه البنية لـ homecode دعم عدة عملاء وتمكّنك من التفاعل مع homecode برمجيا.

يمكنك تشغيل `homecode serve` لبدء خادم مستقل. إذا كانت واجهة homecode في terminal (TUI)
قيد التشغيل، فسيبدأ `homecode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات HomeCode لـ [IDE](/docs/ide).
...
يعرض خادم homecode واجهات API التالية.
