...
مع OpenQCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenQCode سمتنا `openqcode`.
...
يأتي OpenQCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenQCode مظهر terminal لديهم
...
يدعم OpenQCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openqcode/themes/*.json` أو `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openqcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openqcode/themes/*.json`
