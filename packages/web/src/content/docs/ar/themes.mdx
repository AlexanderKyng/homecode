...
مع HomeCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم HomeCode سمتنا `homecode`.
...
يأتي HomeCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق HomeCode مظهر terminal لديهم
...
يدعم HomeCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/homecode/themes/*.json` أو `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.homecode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.homecode/themes/*.json`
