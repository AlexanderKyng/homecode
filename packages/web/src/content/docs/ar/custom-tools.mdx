...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في openqcode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في openqcode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.openqcode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/openqcode/tools/`.
