---
title: Config
description: Using the HomeCode JSON config.
---

You can configure HomeCode using a JSON config file.

---

## Format

HomeCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="homecode.jsonc"
{
...
1. **Remote config** (from `.well-known/homecode`) - organizational defaults
2. **Global config** (`~/.config/homecode/homecode.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`homecode.json` in project) - project-specific settings
5. **`.homecode` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/homecode/` on macOS) - admin-controlled
...
:::note
The `.homecode` and `~/.config/homecode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/homecode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/homecode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global HomeCode config in `~/.config/homecode/homecode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/homecode/tui.json`.
...
Add `homecode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When HomeCode starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
modes, and plugins just like the standard `.homecode` directory, and should
...
The custom directory is loaded after the global config and `.homecode` directories, so it **can override** their settings.
...
Drop an `homecode.json` or `homecode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/homecode/` |
| Linux    | `/etc/homecode/`                         |
| Windows  | `%ProgramData%\homecode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, HomeCode reads managed preferences from the `ai.homecode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

HomeCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.homecode.managed.plist`
2. `/Library/Managed Preferences/ai.homecode.managed.plist`

The plist keys map directly to `homecode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.homecode.managed` PayloadType. The HomeCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.homecode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.homecode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.homecode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`homecode.ai/config.json`**](https://homecode.ai/config.json).

TUI config uses [**`homecode.ai/tui.json`**](https://homecode.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `homecode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `homecode serve` and `homecode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your HomeCode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `homecode.local`. Useful for running multiple instances on the same network.
...
If not specified, HomeCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your HomeCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, HomeCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
HomeCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, HomeCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/homecode/agents/` or `.homecode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, HomeCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`homecode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/homecode/commands/` or `.homecode/commands/`. [Learn more here](/docs/commands).
...
HomeCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
HomeCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, homecode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend HomeCode with custom tools, hooks, and integrations.

Place plugin files in `.homecode/plugins/` or `~/.config/homecode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="homecode.json"
{
  "$schema": "https://homecode.ai/config.json",
  "plugin": ["homecode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict HomeCode to only use specific providers rather than disabling them one by one.
