...
Create `.homecode/commands/test.md`:
...
You can add custom commands through the HomeCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your HomeCode [config](/docs/config):
...
homecode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
