...
Create `.openqcode/commands/test.md`:
...
You can add custom commands through the OpenQCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenQCode [config](/docs/config):
...
- Per-project: `.openqcode/commands/`
...
openqcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
