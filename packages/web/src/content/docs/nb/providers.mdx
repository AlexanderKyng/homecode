---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenQCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenQCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenQCode-konfigurasjonen.
...
i `~/.local/share/openqcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenQCode
konfig.
...
OpenQCode Zen er en liste over modeller levert av OpenQCode-teamet som har vært
testet og verifisert for å fungere godt med OpenQCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenQCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenQCode Zen` og gå til [openqcode.ai/auth](https://openqcode.ai/zen).
...
Det fungerer som alle andre leverandører i OpenQCode og er helt valgfritt å bruke.

---

## OpenQCode Go

OpenQCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenQCode-teamet som har vært
testet og verifisert for å fungere godt med OpenQCode.

1. Kjør kommandoen `/connect` i TUI, velg `OpenQCode Go`, og gå til [openqcode.ai/auth](https://openqcode.ai/zen).
...
Slik bruker du Amazon Bedrock med OpenQCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenQCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `openqcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openqcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openqcode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenQCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenQCode-konfigurasjonen.
...
for å lagre token i OpenQCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenQCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenQCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`openqcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `openqcode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenQCode:
...
Slik bruker du Google Vertex AI med OpenQCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenQCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenQCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [openqcode-helicone-session](https://github.com/H2Shami/openqcode-helicone-session) plugin for automatisk å logge hver OpenQCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenQCode-samtale oppført som en separat økt.
...
Du kan konfigurere OpenQCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenQCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenQCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenQCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openqcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenQCode:
...
3. Kopier API-nøkkelen for bruk i OpenQCode.
...
6. **Viktig**: Før du bruker skymodeller i OpenQCode, må du hente modellinformasjonen lokalt:
...
OpenQCode Zen er en liste over testede og verifiserte modeller levert av OpenQCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenQCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenQCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenQCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenQCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenQCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenQCode:
...
Du kan også tilpasse modeller gjennom OpenQCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenQCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `openqcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenQCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openqcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenQCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenQCode-konfigurasjonen.
