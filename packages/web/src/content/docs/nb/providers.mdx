---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i HomeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

HomeCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i HomeCode-konfigurasjonen.
...
i `~/.local/share/homecode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i HomeCode
konfig.
...
HomeCode Zen er en liste over modeller levert av HomeCode-teamet som har vært
testet og verifisert for å fungere godt med HomeCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med HomeCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `HomeCode Zen` og gå til [homecode.ai/auth](https://homecode.ai/zen).
...
Det fungerer som alle andre leverandører i HomeCode og er helt valgfritt å bruke.

---

## HomeCode Go

HomeCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av HomeCode-teamet som har vært
testet og verifisert for å fungere godt med HomeCode.

1. Kjør kommandoen `/connect` i TUI, velg `HomeCode Go`, og gå til [homecode.ai/auth](https://homecode.ai/zen).
...
Slik bruker du Amazon Bedrock med HomeCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører HomeCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `homecode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i homecode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere homecode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at HomeCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom HomeCode-konfigurasjonen.
...
for å lagre token i HomeCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
HomeCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse HomeCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`homecode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med HomeCode:
...
Slik bruker du Google Vertex AI med HomeCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører HomeCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom HomeCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [homecode-helicone-session](https://github.com/H2Shami/homecode-helicone-session) plugin for automatisk å logge hver HomeCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver HomeCode-samtale oppført som en separat økt.
...
Du kan konfigurere HomeCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere HomeCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere HomeCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for HomeCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/homecode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med HomeCode:
...
3. Kopier API-nøkkelen for bruk i HomeCode.
...
6. **Viktig**: Før du bruker skymodeller i HomeCode, må du hente modellinformasjonen lokalt:
...
HomeCode Zen er en liste over testede og verifiserte modeller levert av HomeCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>HomeCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **HomeCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn HomeCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom HomeCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom HomeCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med HomeCode:
...
Du kan også tilpasse modeller gjennom HomeCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med HomeCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `homecode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar HomeCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `homecode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk HomeCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i HomeCode-konfigurasjonen.
