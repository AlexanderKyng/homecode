...
description: Skriv dine egne programtillegg for å utvide OpenQCode.
---

Plugins lar deg utvide OpenQCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenQCode.
...
- `.openqcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/openqcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/openqcode/openqcode.json`)
2. Prosjektkonfigurasjon (`openqcode.json`)
3. Global plugin-katalog (`~/.config/openqcode/plugins/`)
4. Prosjektpluginkatalog (`.openqcode/plugins/`)
...
OpenQCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenQCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenQCode.
...
:::note
Hvis du bruker OpenQCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenQCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenQCode:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenQCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenQCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://openqcode.ai/docs/sdk) for detaljer.
