...
description: Skriv dine egne programtillegg for å utvide HomeCode.
---

Plugins lar deg utvide HomeCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til HomeCode.
...
- `.homecode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/homecode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/homecode/homecode.json`)
2. Prosjektkonfigurasjon (`homecode.json`)
3. Global plugin-katalog (`~/.config/homecode/plugins/`)
4. Prosjektpluginkatalog (`.homecode/plugins/`)
...
HomeCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En HomeCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide HomeCode.
...
:::note
Hvis du bruker HomeCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre HomeCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til HomeCode:
...
`tool`-hjelperen lager et tilpasset verktøy som HomeCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for HomeCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://homecode.ai/docs/sdk) for detaljer.
