---
title: CLI
description: OpenQCode CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenQCode CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
openqcode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med OpenQCode programmatisk.
...
Start OpenQCode-terminalbrukergrensesnittet.
...
Administrer agenter for OpenQCode.
...
Koble en terminal til en allerede kjørende OpenQCode-backend-server startet via `serve` eller `web` kommandoer.

```bash
openqcode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern OpenQCode-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `openqcode`) |
...
OpenQCode drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `openqcode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

Når OpenQCode starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i OpenQCode.
...
Kjør OpenQCode i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `openqcode serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende OpenQCode-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `openqcode`) |
...
Start en headless OpenQCode-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
openqcode serve
```

Dette starter en HTTP-server som gir API tilgang til OpenQCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `openqcode`).
...
Vis tokenbruk og kostnadsstatistikk for OpenQCode-øktene dine.
...
Importer øktdata fra en JSON-fil eller en OpenQCode share-URL.
...
Start en headless OpenQCode-server med et webgrensesnitt.

```bash
openqcode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til OpenQCode via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `openqcode`).
...
Avinstaller OpenQCode og fjern alle relaterte filer.
...
Oppdaterer OpenQCode til siste versjon eller en spesifikk versjon.
...
OpenQCode CLI bruker følgende globale flagg.
...
OpenQCode kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `openqcode`) |
