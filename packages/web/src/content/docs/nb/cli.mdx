---
title: CLI
description: HomeCode CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

HomeCode CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
homecode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med HomeCode programmatisk.
...
Start HomeCode-terminalbrukergrensesnittet.
...
Administrer agenter for HomeCode.
...
Koble en terminal til en allerede kjørende HomeCode-backend-server startet via `serve` eller `web` kommandoer.

```bash
homecode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern HomeCode-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `homecode`) |
...
HomeCode drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `homecode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

Når HomeCode starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i HomeCode.
...
Kjør HomeCode i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `homecode serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende HomeCode-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `homecode`) |
...
Start en headless HomeCode-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
homecode serve
```

Dette starter en HTTP-server som gir API tilgang til HomeCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `homecode`).
...
Vis tokenbruk og kostnadsstatistikk for HomeCode-øktene dine.
...
Importer øktdata fra en JSON-fil eller en HomeCode share-URL.
...
Start en headless HomeCode-server med et webgrensesnitt.

```bash
homecode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til HomeCode via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `homecode`).
...
Avinstaller HomeCode og fjern alle relaterte filer.
...
Oppdaterer HomeCode til siste versjon eller en spesifikk versjon.
...
HomeCode CLI bruker følgende globale flagg.
...
HomeCode kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `homecode`) |
