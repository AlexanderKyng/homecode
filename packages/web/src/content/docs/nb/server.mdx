---
title: Tjener
description: Samhandle med HomeCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `homecode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en HomeCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `homecode.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `homecode` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `homecode serve` og `homecode web`.
...
Når du kjører `homecode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk HomeCode-serveren til å samhandle med HomeCode programmatisk.
:::

Denne arkitekturen lar HomeCode støtte flere klienter og lar deg samhandle med HomeCode programmatisk.

Du kan kjøre `homecode serve` for å starte en frittstående server. Hvis du har
HomeCode TUI kjører, vil `homecode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av HomeCode [IDE](/docs/ide) plugins.
...
homecode-serveren viser følgende APIer.
