---
title: Konfigurasjon
description: Ved å bruke openqcode JSON-konfigurasjonen.
---

Du kan konfigurere OpenQCode ved å bruke en JSON konfigurasjonsfil.

---

## Format

OpenQCode støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "openqcode"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/openqcode`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/openqcode/openqcode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`openqcode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.openqcode` kataloger** - agenter, kommandoer, plugins
...
:::note
`.openqcode` og `~/.config/openqcode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/openqcode`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale OpenQCode-konfigurasjon i `~/.config/openqcode/openqcode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `openqcode.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når OpenQCode starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
moduser og plugins akkurat som standard `.openqcode` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.openqcode`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`openqcode.ai/config.json`**](https://openqcode.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `openqcode serve` og `openqcode web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage OpenQCode-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `openqcode.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i OpenQCode-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver OpenQCode å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/openqcode/agents/` eller `.openqcode/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil OpenQCode falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`openqcode run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/openqcode/commands/` eller `.openqcode/commands/`. [Les mer her](/docs/commands).
...
OpenQCode vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater OpenQCode alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider OpenQCode med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.openqcode/plugins/` eller `~/.config/openqcode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense OpenQCode til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
