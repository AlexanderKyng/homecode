---
title: ACP-støtte
description: Bruk homecode i en hvilken som helst ACP-kompatibel editor.
---

HomeCode støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke HomeCode via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `homecode acp`.

Kommandoen starter HomeCode som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'HomeCode'-agenten i AI Chat-agentvelgeren.
...
For å bruke HomeCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke HomeCode som ACP-agent for chat.
...
HomeCode fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i HomeCode-konfigurasjonen
