---
title: ACP-støtte
description: Bruk openqcode i en hvilken som helst ACP-kompatibel editor.
---

OpenQCode støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke OpenQCode via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `openqcode acp`.

Kommandoen starter OpenQCode som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'OpenQCode'-agenten i AI Chat-agentvelgeren.
...
For å bruke OpenQCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke OpenQCode som ACP-agent for chat.
...
OpenQCode fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i OpenQCode-konfigurasjonen
