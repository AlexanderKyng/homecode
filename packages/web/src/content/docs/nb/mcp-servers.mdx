...
Du kan legge til eksterne verktøy i HomeCode ved å bruke _Model Context Protocol_ eller MCP. HomeCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [HomeCode Config](https://homecode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="homecode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/homecode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
HomeCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil HomeCode:
...
Hvis serveren krever autentisering, vil HomeCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `homecode mcp auth <server-name>`.
...
```bash
homecode mcp auth my-oauth-server
```
...
```bash
homecode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil HomeCode lagre tokens sikkert i `~/.local/share/homecode/mcp-auth.json`.
...
homecode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i HomeCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom HomeCode-konfigurasjonen som alle andre verktøy.
...
```json title="homecode.json" {11, 14-18}
{
  "$schema": "https://homecode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
homecode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble HomeCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver HomeCode-samtale oppført som en separat økt.
