...
Du kan legge til eksterne verktøy i OpenQCode ved å bruke _Model Context Protocol_ eller MCP. OpenQCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenQCode Config](https://openqcode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="openqcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/openqcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="openqcode.jsonc" {15}
{
  "$schema": "https://openqcode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenQCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenQCode:
...
Hvis serveren krever autentisering, vil OpenQCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `openqcode mcp auth <server-name>`.
...
```json title="openqcode.json" {7-11}
{
  "$schema": "https://openqcode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
openqcode mcp auth my-oauth-server
```
...
```bash
openqcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenQCode lagre tokens sikkert i `~/.local/share/openqcode/mcp-auth.json`.
...
openqcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenQCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenQCode-konfigurasjonen som alle andre verktøy.
...
```json title="openqcode.json" {11, 14-18}
{
  "$schema": "https://openqcode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
openqcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenQCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenQCode-samtale oppført som en separat økt.
