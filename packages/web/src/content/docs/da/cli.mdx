---
title: CLI
description: HomeCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

HomeCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
homecode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med HomeCode programmatisk.
...
Start HomeCode terminalbrugergrænsefladen.
...
HomeCode CLI har også følgende kommandoer.

---

### agent

Administrer agenter i HomeCode.
...
Tilslut en terminal til en allerede kørende HomeCode backend-server startet via `serve` eller `web` kommandoer.

```bash
homecode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern HomeCode backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `homecode`) |
...
HomeCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `homecode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

Når HomeCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i HomeCode.
...
Kør homecode i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `homecode serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende homecode-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `homecode`)   |
...
Start en hovedløs HomeCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
homecode serve
```

Dette starter en HTTP-server, der giver API-adgang til homecode-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `homecode`).
...
Administrator HomeCode sessionsholder.
...
Liste over alle HomeCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine HomeCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller HomeCode del URL.

```bash
homecode import <file>
```

Du kan importere fra en lokal fil eller en HomeCode share URL.
...
Start en hovedløs HomeCode-server med en webgrænseflade.

```bash
homecode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til HomeCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `homecode`).
...
Afinstaller HomeCode og fjern alle relaterede filer.
...
Opdaterer HomeCode til den nyeste version eller en specifik version.
...
homecode CLI tager følgende globale flag.
...
HomeCode kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `homecode`) |
