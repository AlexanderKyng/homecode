---
title: CLI
description: OpenQCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenQCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
openqcode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med OpenQCode programmatisk.
...
Start OpenQCode terminalbrugergrænsefladen.
...
OpenQCode CLI har også følgende kommandoer.

---

### agent

Administrer agenter i OpenQCode.
...
Tilslut en terminal til en allerede kørende OpenQCode backend-server startet via `serve` eller `web` kommandoer.

```bash
openqcode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern OpenQCode backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `openqcode`) |
...
OpenQCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `openqcode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

Når OpenQCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i OpenQCode.
...
Kør openqcode i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `openqcode serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende openqcode-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `openqcode`)   |
...
Start en hovedløs OpenQCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
openqcode serve
```

Dette starter en HTTP-server, der giver API-adgang til openqcode-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `openqcode`).
...
Administrator OpenQCode sessionsholder.
...
Liste over alle OpenQCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine OpenQCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller OpenQCode del URL.

```bash
openqcode import <file>
```

Du kan importere fra en lokal fil eller en OpenQCode share URL.
...
Start en hovedløs OpenQCode-server med en webgrænseflade.

```bash
openqcode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenQCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `openqcode`).
...
Afinstaller OpenQCode og fjern alle relaterede filer.
...
Opdaterer OpenQCode til den nyeste version eller en specifik version.
...
openqcode CLI tager følgende globale flag.
...
OpenQCode kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `openqcode`) |
