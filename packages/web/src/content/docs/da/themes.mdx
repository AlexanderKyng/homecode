...
Med HomeCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger HomeCode vårt eget `homecode`-tema.
...
HomeCode kommer med flere innebygde temaer.
...
- Vil at HomeCode skal matche terminalens utseende
...
HomeCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/homecode/themes/*.json` eller `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.homecode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.homecode/themes/*.json`
