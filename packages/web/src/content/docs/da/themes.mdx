...
Med OpenQCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenQCode vårt eget `openqcode`-tema.
...
OpenQCode kommer med flere innebygde temaer.
...
- Vil at OpenQCode skal matche terminalens utseende
...
OpenQCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openqcode/themes/*.json` eller `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openqcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openqcode/themes/*.json`
