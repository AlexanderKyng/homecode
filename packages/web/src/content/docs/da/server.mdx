---
title: Server
description: Interager med openqcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openqcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openqcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openqcode.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openqcode`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openqcode serve` og `openqcode web`.
...
Når du kører `openqcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openqcode-serveren til at interagere med openqcode programmatisk.
:::

Denne arkitektur lader openqcode understøtte flere klienter og lader dig interagere med openqcode programmatisk.

Du kan køre `openqcode serve` for at starte en selvstændig server. Hvis du har
openqcode TUI kørende, vil `openqcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenQCode [IDE](/docs/ide) plugins.
...
OpenQCode-serveren viser følgende API'er.
