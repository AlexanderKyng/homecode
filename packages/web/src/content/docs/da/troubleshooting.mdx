...
For at fejlfinde problemer med HomeCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\homecode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `homecode --log-level DEBUG`.

---

## Lagring

homecode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/homecode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\homecode`
...
HomeCode Desktop kører en lokal HomeCode-server (`homecode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `HomeCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/homecode/homecode.jsonc` (eller `~/.config/homecode/homecode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/homecode/homecode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\homecode\homecode.jsonc`
...
HomeCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/homecode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\homecode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så HomeCode kan genopbygge den.

1. Afslut HomeCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/homecode`
- **Linux**: slet `~/.cache/homecode` (eller kør `rm -rf ~/.cache/homecode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\homecode`

3. Start HomeCode Desktop på ny.

---

### Løs problemer med serverforbindelse

HomeCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `homecode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver HomeCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i HomeCode.

---

### Meddelelser vises ikke

HomeCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for HomeCode i dine OS-indstillinger, og
...
1. Afslut HomeCode Desktop.
2. Find og slet disse filer (de findes i HomeCode Desktop-appens datamappe):

- `homecode.settings.dat` (skrivebordsstandardserver URL)
- `homecode.global.dat` og `homecode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med HomeCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `homecode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\homecode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. homecode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\homecode`

2. Start homecode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge homecode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
homecode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
