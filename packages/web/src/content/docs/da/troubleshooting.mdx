...
For at fejlfinde problemer med OpenQCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openqcode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `openqcode --log-level DEBUG`.

---

## Lagring

openqcode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/openqcode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openqcode`
...
OpenQCode Desktop kører en lokal OpenQCode-server (`openqcode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenQCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/openqcode/openqcode.jsonc` (eller `~/.config/openqcode/openqcode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/openqcode/openqcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openqcode\openqcode.jsonc`
...
OpenQCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/openqcode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openqcode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenQCode kan genopbygge den.

1. Afslut OpenQCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/openqcode`
- **Linux**: slet `~/.cache/openqcode` (eller kør `rm -rf ~/.cache/openqcode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\openqcode`

3. Start OpenQCode Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenQCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `openqcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver OpenQCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenQCode.

---

### Meddelelser vises ikke

OpenQCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenQCode i dine OS-indstillinger, og
...
1. Afslut OpenQCode Desktop.
2. Find og slet disse filer (de findes i OpenQCode Desktop-appens datamappe):

- `openqcode.settings.dat` (skrivebordsstandardserver URL)
- `openqcode.global.dat` og `openqcode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OpenQCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `openqcode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\openqcode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. openqcode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\openqcode`

2. Start openqcode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge openqcode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
openqcode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
