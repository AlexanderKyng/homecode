---
title: Plugins
description: Skriv dine egne plugins for at udvide HomeCode.
---

Plugins giver dig mulighed for at udvide HomeCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre HomeCodes standardadfærd.
...
- `.homecode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/homecode/node_modules/`.
...
1. Global konfiguration (`~/.config/homecode/homecode.json`)
2. Projektkonfiguration (`homecode.json`)
3. Global plugin-mappe (`~/.config/homecode/plugins/`)
4. Projekt plugin bibliotek (`.homecode/plugins/`)
...
HomeCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En homecode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide homecode.
...
:::note
Hvis du bruger HomeCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå homecode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til homecode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som homecode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for homecode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://homecode.ai/docs/sdk) for detaljer.
