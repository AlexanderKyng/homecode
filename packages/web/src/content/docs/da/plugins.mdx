---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenQCode.
---

Plugins giver dig mulighed for at udvide OpenQCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenQCodes standardadfærd.
...
- `.openqcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openqcode/node_modules/`.
...
1. Global konfiguration (`~/.config/openqcode/openqcode.json`)
2. Projektkonfiguration (`openqcode.json`)
3. Global plugin-mappe (`~/.config/openqcode/plugins/`)
4. Projekt plugin bibliotek (`.openqcode/plugins/`)
...
OpenQCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En openqcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openqcode.
...
:::note
Hvis du bruger OpenQCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå openqcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openqcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openqcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openqcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://openqcode.ai/docs/sdk) for detaljer.
