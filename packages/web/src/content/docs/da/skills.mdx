...
Agentferdigheter lar OpenQCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
OpenQCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.openqcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openqcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går OpenQCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openqcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openqcode/skills/git-release/SKILL.md` slik:
...
OpenQCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `openqcode.json`:
...
**For innebygde agenter** (i `openqcode.json`):
