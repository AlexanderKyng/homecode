...
Agentferdigheter lar HomeCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
HomeCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.homecode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/homecode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går HomeCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.homecode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/homecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.homecode/skills/git-release/SKILL.md` slik:
...
HomeCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `homecode.json`:
...
**For innebygde agenter** (i `homecode.json`):
