---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenQCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenQCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenQCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/openqcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenQCode-konfigurationen.
...
OpenQCode Zen er en liste over modeller leveret af OpenQCode-teamet, som er blevet testet og verificeret til at fungere godt med OpenQCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenQCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg openqcode og gå til [openqcode.ai/auth](https://openqcode.ai/auth).
...
Det fungerer som alle andre udbydere i OpenQCode og er helt valgfrit at bruge.

---

## OpenQCode Go

OpenQCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenQCode-teamet, som er testet og verificeret til at fungere godt med OpenQCode.

1. Kør kommandoen `/connect` i TUI, vælg `OpenQCode Go`, og gå til [openqcode.ai/auth](https://openqcode.ai/zen).
...
Sådan bruger du Amazon Bedrock med OpenQCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører openqcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `openqcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenQCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openqcode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at openqcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem openqcode-konfigurationen.
...
gemme tokenet i openqcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenQCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenQCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`openqcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med OpenQCode:
...
Sådan bruger du Google Vertex AI med OpenQCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører openqcode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem openqcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [openqcode-helicone-session](https://github.com/H2Shami/openqcode-helicone-session) plugin for automatisk at logge hver OpenQCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenQCode-samtale opført som en separat session.
...
Du kan konfigurere openqcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere openqcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere openqcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenQCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/openqcode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenQCode:
...
3. Kopier API-nøglen til brug i OpenQCode.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenQCode, skal du hente modelinformationen lokalt:
...
OpenQCode Zen er en liste over testede og verificerede modeller leveret af OpenQCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenQCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenQCode Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenQCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem openqcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem openqcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenQCode:
...
Du kan også tilpasse modeller gennem openqcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med openqcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `openqcode.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenQCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `openqcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek openqcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i openqcode-konfigurationen.
