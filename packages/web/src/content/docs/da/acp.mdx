...
description: Brug OpenQCode i enhver ACP-kompatibel editor.
---

OpenQCode understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge OpenQCode via ACP, konfigurer din editor til at køre kommandoen `openqcode acp`.

Kommandoen starter OpenQCode som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'OpenQCode'-agent i AI Chat-agentvælgeren.
...
For at bruge OpenQCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge OpenQCode som ACP-agent til chat.
...
OpenQCode fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din OpenQCode-konfiguration
