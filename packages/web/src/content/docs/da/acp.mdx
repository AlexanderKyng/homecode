...
description: Brug HomeCode i enhver ACP-kompatibel editor.
---

HomeCode understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge HomeCode via ACP, konfigurer din editor til at køre kommandoen `homecode acp`.

Kommandoen starter HomeCode som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'HomeCode'-agent i AI Chat-agentvælgeren.
...
For at bruge HomeCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge HomeCode som ACP-agent til chat.
...
HomeCode fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din HomeCode-konfiguration
