---
title: LSP Servers
description: OpenQCode integrates with your LSP servers.
---

OpenQCode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

OpenQCode comes with several built-in LSP servers for popular languages:
...
When LSP is enabled and openqcode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your openqcode config.
...
```json title="openqcode.json" {5-13}
{
  "$schema": "https://openqcode.ai/config.json",
  "lsp": {
    "custom-lsp": {
