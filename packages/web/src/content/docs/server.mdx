---
title: Server
description: Interact with homecode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `homecode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an homecode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `homecode.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `homecode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `homecode serve` and `homecode web`.
...
When you run `homecode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the homecode server to interact with homecode programmatically.
:::

This architecture lets homecode support multiple clients and allows you to interact with homecode programmatically.

You can run `homecode serve` to start a standalone server. If you have the
homecode TUI running, `homecode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the HomeCode [IDE](/docs/ide) plugins.
...
The homecode server exposes the following APIs.
