---
title: Server
description: Interact with openqcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openqcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openqcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `openqcode.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openqcode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `openqcode serve` and `openqcode web`.
...
When you run `openqcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openqcode server to interact with openqcode programmatically.
:::

This architecture lets openqcode support multiple clients and allows you to interact with openqcode programmatically.

You can run `openqcode serve` to start a standalone server. If you have the
openqcode TUI running, `openqcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenQCode [IDE](/docs/ide) plugins.
...
The openqcode server exposes the following APIs.
