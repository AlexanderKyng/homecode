...
With OpenQCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenQCode uses our own `openqcode` theme.
...
OpenQCode comes with several built-in themes.
...
- Want OpenQCode to match their terminal's appearance
...
OpenQCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/openqcode/themes/*.json` or `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openqcode/themes/*.json`
4. **Current working directory** - Defined in `./.openqcode/themes/*.json`
