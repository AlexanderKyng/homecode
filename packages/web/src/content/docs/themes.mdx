...
With HomeCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, HomeCode uses our own `homecode` theme.
...
HomeCode comes with several built-in themes.
...
- Want HomeCode to match their terminal's appearance
...
HomeCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/homecode/themes/*.json` or `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.homecode/themes/*.json`
4. **Current working directory** - Defined in `./.homecode/themes/*.json`
