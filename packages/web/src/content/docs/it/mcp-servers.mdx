...
Puoi aggiungere strumenti esterni a HomeCode usando il _Model Context Protocol_, o MCP. HomeCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [HomeCode Config](https://homecode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/homecode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
HomeCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, HomeCode:
...
Se il server richiede autenticazione, HomeCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `homecode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, HomeCode memorizzera' i token in modo sicuro in `~/.local/share/homecode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in HomeCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione HomeCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare HomeCode al tuo account Sentry.
