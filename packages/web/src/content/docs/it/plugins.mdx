---
title: Plugin
description: Scrivi plugin per estendere OpenQCode.
---

I plugin ti permettono di estendere OpenQCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenQCode.
...
- `.openqcode/plugins/` - plugin a livello progetto
- `~/.config/openqcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openqcode/node_modules/`.
...
1. Config globale (`~/.config/openqcode/openqcode.json`)
2. Config di progetto (`openqcode.json`)
3. Directory plugin globale (`~/.config/openqcode/plugins/`)
4. Directory plugin di progetto (`.openqcode/plugins/`)
...
OpenQCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di openqcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openqcode.
...
:::note
Se usi l'app desktop di OpenQCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a openqcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openqcode:
...
L'helper `tool` crea uno strumento personalizzato che openqcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openqcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://openqcode.ai/docs/sdk) per i dettagli.
