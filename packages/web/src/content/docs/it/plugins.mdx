---
title: Plugin
description: Scrivi plugin per estendere HomeCode.
---

I plugin ti permettono di estendere HomeCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di HomeCode.
...
- `.homecode/plugins/` - plugin a livello progetto
- `~/.config/homecode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/homecode/node_modules/`.
...
1. Config globale (`~/.config/homecode/homecode.json`)
2. Config di progetto (`homecode.json`)
3. Directory plugin globale (`~/.config/homecode/plugins/`)
4. Directory plugin di progetto (`.homecode/plugins/`)
...
HomeCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di homecode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere homecode.
...
:::note
Se usi l'app desktop di HomeCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a homecode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a homecode:
...
L'helper `tool` crea uno strumento personalizzato che homecode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in homecode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://homecode.ai/docs/sdk) per i dettagli.
