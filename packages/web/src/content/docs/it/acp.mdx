---
title: Supporto ACP
description: Usa OpenQCode in qualunque editor compatibile con ACP.
---

OpenQCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare OpenQCode via ACP, configura l'editor in modo che esegua il comando `openqcode acp`.

Il comando avvia OpenQCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'OpenQCode' nel selettore agenti di AI Chat.
...
Per usare OpenQCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare OpenQCode come agente ACP per la chat.
...
OpenQCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione OpenQCode
