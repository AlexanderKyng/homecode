---
title: Supporto ACP
description: Usa HomeCode in qualunque editor compatibile con ACP.
---

HomeCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare HomeCode via ACP, configura l'editor in modo che esegua il comando `homecode acp`.

Il comando avvia HomeCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'HomeCode' nel selettore agenti di AI Chat.
...
Per usare HomeCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare HomeCode come agente ACP per la chat.
...
HomeCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione HomeCode
