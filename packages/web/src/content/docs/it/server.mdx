---
title: Server
description: Interagisci con il server openqcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openqcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openqcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `openqcode.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openqcode`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openqcode serve` sia per `openqcode web`.
...
Quando esegui `openqcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openqcode per interagire con openqcode in modo programmatico.
:::

Questa architettura permette a openqcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openqcode serve` per avviare un server standalone. Se la TUI di openqcode e gia in esecuzione, `openqcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OpenQCode.
...
Il server openqcode espone le seguenti API.
