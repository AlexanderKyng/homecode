...
Le skill degli agenti permettono a OpenQCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenQCode cerca in queste posizioni:

- Config di progetto: `.openqcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/openqcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenQCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.openqcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/openqcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.openqcode/skills/git-release/SKILL.md` cosi':
...
OpenQCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `openqcode.json`:
...
**Per agenti integrati** (in `openqcode.json`):
