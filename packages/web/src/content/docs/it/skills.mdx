...
Le skill degli agenti permettono a HomeCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
HomeCode cerca in queste posizioni:

- Config di progetto: `.homecode/skills/<name>/SKILL.md`
- Config globale: `~/.config/homecode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, HomeCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.homecode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/homecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.homecode/skills/git-release/SKILL.md` cosi':
...
HomeCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `homecode.json`:
...
**Per agenti integrati** (in `homecode.json`):
