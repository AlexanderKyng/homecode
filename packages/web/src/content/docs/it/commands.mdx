...
Crea `.homecode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di HomeCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di HomeCode:
...
- Globale: `~/.config/homecode/commands/`
- Per progetto: `.homecode/commands/`
...
homecode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
