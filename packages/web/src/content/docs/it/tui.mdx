---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di HomeCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

HomeCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire HomeCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di HomeCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a HomeCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da HomeCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `homecode.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo HomeCode attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `homecode.default`.
