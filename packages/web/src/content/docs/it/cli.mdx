---
title: CLI
description: Opzioni e comandi della CLI di HomeCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di HomeCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
homecode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con HomeCode in modo programmatico.
...
Avvia l'interfaccia testuale di HomeCode (interfaccia utente da terminale).
...
La CLI di HomeCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per HomeCode.
...
Collega un terminale a un backend server di HomeCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
homecode attach [url]
```

Questo consente di usare la TUI con un backend HomeCode remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `homecode`) |
...
HomeCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `homecode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

Quando HomeCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in HomeCode.
...
Esegui homecode in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `homecode serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server homecode in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `homecode`) |
...
Avvia un server HomeCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
homecode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di homecode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `homecode`).
...
Elenca tutte le sessioni HomeCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni HomeCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione HomeCode.

```bash
homecode import <file>
```

Puoi importare da un file locale o da un URL di condivisione HomeCode.
...
Avvia un server HomeCode headless con interfaccia web.

```bash
homecode web
```

Avvia un server HTTP e apre un browser per accedere a HomeCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `homecode`).
...
Disinstalla HomeCode e rimuove tutti i file correlati.
...
Aggiorna homecode all'ultima versione o a una versione specifica.
...
La CLI di homecode accetta i seguenti flag globali.
...
HomeCode può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `homecode`)     |
