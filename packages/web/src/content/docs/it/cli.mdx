---
title: CLI
description: Opzioni e comandi della CLI di OpenQCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di OpenQCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
openqcode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con OpenQCode in modo programmatico.
...
Avvia l'interfaccia testuale di OpenQCode (interfaccia utente da terminale).
...
La CLI di OpenQCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per OpenQCode.
...
Collega un terminale a un backend server di OpenQCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
openqcode attach [url]
```

Questo consente di usare la TUI con un backend OpenQCode remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `openqcode`) |
...
OpenQCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `openqcode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

Quando OpenQCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in OpenQCode.
...
Esegui openqcode in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `openqcode serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server openqcode in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `openqcode`) |
...
Avvia un server OpenQCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
openqcode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di openqcode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `openqcode`).
...
Gestisci le sessioni OpenQCode.
...
Elenca tutte le sessioni OpenQCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni OpenQCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione OpenQCode.

```bash
openqcode import <file>
```

Puoi importare da un file locale o da un URL di condivisione OpenQCode.
...
Avvia un server OpenQCode headless con interfaccia web.

```bash
openqcode web
```

Avvia un server HTTP e apre un browser per accedere a OpenQCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `openqcode`).
...
Disinstalla OpenQCode e rimuove tutti i file correlati.
...
Aggiorna openqcode all'ultima versione o a una versione specifica.
...
La CLI di openqcode accetta i seguenti flag globali.
...
OpenQCode può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `openqcode`)     |
