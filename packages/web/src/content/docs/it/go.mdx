...
HomeCode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in HomeCode. Ti abboni a HomeCode Go e
...
HomeCode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

HomeCode Go funziona come qualsiasi altro provider in HomeCode.

1. Accedi a **<a href={console}>HomeCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `HomeCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a HomeCode Go.
:::
...
HomeCode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua HomeCode config
utilizza il formato `homecode-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`homecode-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato HomeCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con HomeCode.
