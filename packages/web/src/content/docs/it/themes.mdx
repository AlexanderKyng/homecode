...
Con OpenQCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OpenQCode usa il tema `openqcode`.
...
OpenQCode include diversi temi integrati.
...
- Vuole che OpenQCode corrisponda all'aspetto del terminale
...
OpenQCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/openqcode/themes/*.json` o `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.openqcode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.openqcode/themes/*.json`
