---
title: Provider
description: Usa qualsiasi provider LLM in OpenQCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenQCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di OpenQCode.
...
OpenQCode Zen e una lista di modelli forniti dal team OpenQCode, testati e
verificati per funzionare bene con OpenQCode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OpenQCode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona openqcode e vai su [openqcode.ai/auth](https://openqcode.ai/auth).
...
Funziona come qualsiasi altro provider in OpenQCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con OpenQCode:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui openqcode:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `openqcode.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in OpenQCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare openqcode per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché openqcode funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di openqcode.
...
salvare il token nello storage di auth di openqcode.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
OpenQCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare OpenQCode
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `openqcode.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `openqcode.json`:
...
Per usare il tuo abbonamento GitHub Copilot con openqcode:
...
Per usare Google Vertex AI con OpenQCode:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui openqcode.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da openqcode, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [openqcode-helicone-session](https://github.com/H2Shami/openqcode-helicone-session) per loggare automaticamente ogni conversazione di OpenQCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di OpenQCode elencata come sessione separata.
...
Puoi configurare openqcode per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare openqcode per usare modelli locali tramite LM Studio.
...
Puoi configurare openqcode per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OpenQCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/openqcode) per dettagli.
:::
...
Per usare Ollama Cloud con OpenQCode:
...
3. Copia la chiave API da usare in OpenQCode.
...
6. **Importante**: Prima di usare modelli cloud in OpenQCode, devi scaricare le informazioni del modello localmente:
...
OpenQCode Zen è una lista di modelli testati e verificati forniti dal team OpenQCode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>OpenQCode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **OpenQCode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di OpenQCode.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di openqcode.
...
5. Puoi anche personalizzarli tramite la configurazione di openqcode. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di openqcode. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con openqcode. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `openqcode.json` nella directory del progetto:
...
I campi `limit` permettono a OpenQCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `openqcode auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di openqcode e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config openqcode.
