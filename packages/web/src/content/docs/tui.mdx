---
title: TUI
description: Using the OpenQCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenQCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenQCode starts the TUI for the current directory.
...
When using the OpenQCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenQCode. Allows you to select from available providers and add their API keys.
...
Exit OpenQCode. _Aliases_: `/quit`, `/q`
...
This is separate from `openqcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OpenQCode waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `openqcode.default`.
