---
title: TUI
description: Using the HomeCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

HomeCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running HomeCode starts the TUI for the current directory.
...
When using the HomeCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to HomeCode. Allows you to select from available providers and add their API keys.
...
Exit HomeCode. _Aliases_: `/quit`, `/q`
...
This is separate from `homecode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long HomeCode waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `homecode.default`.
