...
Создайте `.openqcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию openqcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openqcode [config](/docs/config):
...
- Глобальный: `~/.config/openqcode/commands/`
- Для каждого проекта: `.openqcode/commands/`
...
openqcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
