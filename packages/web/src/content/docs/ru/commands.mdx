...
Создайте `.homecode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию homecode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем homecode [config](/docs/config):
...
- Глобальный: `~/.config/homecode/commands/`
- Для каждого проекта: `.homecode/commands/`
...
homecode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
