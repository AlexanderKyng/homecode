---
title: CLI
description: HomeCode CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The HomeCode CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
homecode
```

But it also accepts commands as documented on this page. This allows you to interact with HomeCode programmatically.
...
Start the HomeCode terminal user interface.
...
The HomeCode CLI also has the following commands.

---

### agent

Manage agents for HomeCode.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.homecode/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running HomeCode backend server started via `serve` or `web` commands.

```bash
homecode attach [url]
```

This allows using the TUI with a remote HomeCode backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `homecode`) |
...
HomeCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `homecode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/homecode/auth.json`.

```bash
homecode auth login
```

When HomeCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in HomeCode.
...
Run homecode in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `homecode serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>                       |       | Attach to a running homecode server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr>                     | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr>                     | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `homecode`) |
...
Start a headless HomeCode server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
homecode serve
```

This starts an HTTP server that provides API access to homecode functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `homecode`).
...
Delete an HomeCode session.

```bash
homecode session delete <sessionID>
```
...
Show token usage and cost statistics for your HomeCode sessions.
...
Import session data from a JSON file or HomeCode share URL.

```bash
homecode import <file>
```

You can import from a local file or an HomeCode share URL.
...
Start a headless HomeCode server with a web interface.

```bash
homecode web
```

This starts an HTTP server and opens a web browser to access HomeCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `homecode`).
...
```bash
homecode plugin <module>
```

Or use the alias.

```bash
homecode plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run HomeCode.
...
Updates homecode to the latest version or a specific version.
...
The homecode CLI takes the following global flags.
...
HomeCode can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `homecode`) |
