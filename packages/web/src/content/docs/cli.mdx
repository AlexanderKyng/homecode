---
title: CLI
description: OpenQCode CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The OpenQCode CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
openqcode
```

But it also accepts commands as documented on this page. This allows you to interact with OpenQCode programmatically.
...
Start the OpenQCode terminal user interface.
...
The OpenQCode CLI also has the following commands.

---

### agent

Manage agents for OpenQCode.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.openqcode/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running OpenQCode backend server started via `serve` or `web` commands.

```bash
openqcode attach [url]
```

This allows using the TUI with a remote OpenQCode backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `openqcode`) |
...
OpenQCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `openqcode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/openqcode/auth.json`.

```bash
openqcode auth login
```

When OpenQCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in OpenQCode.
...
Run openqcode in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `openqcode serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>                       |       | Attach to a running openqcode server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr>                     | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr>                     | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `openqcode`) |
...
Start a headless OpenQCode server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
openqcode serve
```

This starts an HTTP server that provides API access to openqcode functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `openqcode`).
...
Delete an OpenQCode session.

```bash
openqcode session delete <sessionID>
```
...
Show token usage and cost statistics for your OpenQCode sessions.
...
Import session data from a JSON file or OpenQCode share URL.

```bash
openqcode import <file>
```

You can import from a local file or an OpenQCode share URL.
...
Start a headless OpenQCode server with a web interface.

```bash
openqcode web
```

This starts an HTTP server and opens a web browser to access OpenQCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `openqcode`).
...
```bash
openqcode plugin <module>
```

Or use the alias.

```bash
openqcode plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run OpenQCode.
...
Uninstall OpenQCode and remove all related files.
...
Updates openqcode to the latest version or a specific version.
...
The openqcode CLI takes the following global flags.
...
OpenQCode can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `openqcode`) |
