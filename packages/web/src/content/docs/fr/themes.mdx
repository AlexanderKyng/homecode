...
Avec OpenQCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenQCode utilise notre propre thème `openqcode`.
...
OpenQCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenQCode corresponde à l'apparence de leur terminal
...
OpenQCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openqcode/themes/*.json` ou `$XDG_CONFIG_HOME/openqcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openqcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openqcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openqcode/themes
vim ~/.config/openqcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .openqcode/themes
vim .openqcode/themes/my-theme.json
```
...
{
  "$schema": "https://openqcode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
