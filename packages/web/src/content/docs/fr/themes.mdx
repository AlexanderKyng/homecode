...
Avec HomeCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, HomeCode utilise notre propre thème `homecode`.
...
HomeCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que HomeCode corresponde à l'apparence de leur terminal
...
HomeCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/homecode/themes/*.json` ou `$XDG_CONFIG_HOME/homecode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.homecode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.homecode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/homecode/themes
vim ~/.config/homecode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .homecode/themes
vim .homecode/themes/my-theme.json
```
...
{
  "$schema": "https://homecode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
