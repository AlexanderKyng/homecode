...
Pour déboguer les problèmes avec HomeCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\homecode\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `homecode --log-level DEBUG`.

---

## Stockage

homecode stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/homecode/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\homecode`
...
HomeCode Desktop exécute un serveur HomeCode local (le side-car `homecode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `HomeCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/homecode/homecode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\homecode\homecode.jsonc`
...
HomeCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/homecode/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\homecode\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que HomeCode puisse le reconstruire.

1. Quittez complètement HomeCode Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/homecode`
- **Linux** : supprimez `~/.cache/homecode` (ou exécutez `rm -rf ~/.cache/homecode`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\homecode`

3. Redémarrez le bureau HomeCode.

---

### Résoudre les problèmes de connexion au serveur

HomeCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `homecode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, HomeCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de HomeCode.

---

### Les notifications ne s'affichent pas

HomeCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour HomeCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau HomeCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application HomeCode Desktop) :

- `homecode.settings.dat` (serveur de bureau par défaut URL)
- `homecode.global.dat` et `homecode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec HomeCode :
...
3. Assurez-vous d'avoir la dernière version avec `homecode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `homecode models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\homecode`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. homecode installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\homecode`

2. Redémarrez homecode pour réinstaller les derniers packages du fournisseur

Cela forcera homecode à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
homecode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
