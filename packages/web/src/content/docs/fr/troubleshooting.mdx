...
Pour déboguer les problèmes avec OpenQCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\openqcode\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `openqcode --log-level DEBUG`.

---

## Stockage

openqcode stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/openqcode/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\openqcode`
...
OpenQCode Desktop exécute un serveur OpenQCode local (le side-car `openqcode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `OpenQCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/openqcode/openqcode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\openqcode\openqcode.jsonc`
...
OpenQCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/openqcode/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\openqcode\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que OpenQCode puisse le reconstruire.

1. Quittez complètement OpenQCode Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/openqcode`
- **Linux** : supprimez `~/.cache/openqcode` (ou exécutez `rm -rf ~/.cache/openqcode`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\openqcode`

3. Redémarrez le bureau OpenQCode.

---

### Résoudre les problèmes de connexion au serveur

OpenQCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `openqcode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, OpenQCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de OpenQCode.

---

### Les notifications ne s'affichent pas

OpenQCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour OpenQCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau OpenQCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application OpenQCode Desktop) :

- `openqcode.settings.dat` (serveur de bureau par défaut URL)
- `openqcode.global.dat` et `openqcode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec OpenQCode :
...
3. Assurez-vous d'avoir la dernière version avec `openqcode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `openqcode models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\openqcode`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. openqcode installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\openqcode`

2. Redémarrez openqcode pour réinstaller les derniers packages du fournisseur

Cela forcera openqcode à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
openqcode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
