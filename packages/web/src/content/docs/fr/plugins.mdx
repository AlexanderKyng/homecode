---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenQCode.
---

Les plugins vous permettent d'étendre OpenQCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenQCode.
...
- `.openqcode/plugins/` - Plugins au niveau du projet
- `~/.config/openqcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openqcode/node_modules/`.
...
1. Configuration globale (`~/.config/openqcode/openqcode.json`)
2. Configuration du projet (`openqcode.json`)
3. Répertoire global des plugins (`~/.config/openqcode/plugins/`)
4. Répertoire des plugins du projet (`.openqcode/plugins/`)
...
OpenQCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK openqcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openqcode.
...
:::note
Si vous utilisez l'application de bureau OpenQCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez openqcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openqcode :
...
L'assistant `tool` crée un outil personnalisé que openqcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openqcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://openqcode.ai/docs/sdk) pour plus de détails.
