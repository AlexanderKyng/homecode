---
title: Plugins
description: Écrivez vos propres plugins pour étendre HomeCode.
---

Les plugins vous permettent d'étendre HomeCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de HomeCode.
...
- `.homecode/plugins/` - Plugins au niveau du projet
- `~/.config/homecode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/homecode/node_modules/`.
...
1. Configuration globale (`~/.config/homecode/homecode.json`)
2. Configuration du projet (`homecode.json`)
3. Répertoire global des plugins (`~/.config/homecode/plugins/`)
4. Répertoire des plugins du projet (`.homecode/plugins/`)
...
HomeCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK homecode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre homecode.
...
:::note
Si vous utilisez l'application de bureau HomeCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez homecode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à homecode :
...
L'assistant `tool` crée un outil personnalisé que homecode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour homecode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://homecode.ai/docs/sdk) pour plus de détails.
