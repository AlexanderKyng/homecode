---
title: Serveur
description: Interagissez avec le serveur openqcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openqcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openqcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openqcode.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openqcode`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openqcode serve` et à `openqcode web`.
...
Lorsque vous exécutez `openqcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openqcode pour interagir avec openqcode par programmation.
:::

Cette architecture permet à openqcode de prendre en charge plusieurs clients et vous permet d'interagir avec openqcode par programme.

Vous pouvez exécuter `openqcode serve` pour démarrer un serveur autonome. Si vous avez le
openqcode TUI en cours d'exécution, `openqcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenQCode [IDE](/docs/ide).
...
Le serveur openqcode expose les API suivantes.
