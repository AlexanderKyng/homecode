---
title: Serveur
description: Interagissez avec le serveur homecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `homecode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client homecode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `homecode.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `homecode`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `homecode serve` et à `homecode web`.
...
Lorsque vous exécutez `homecode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur homecode pour interagir avec homecode par programmation.
:::

Cette architecture permet à homecode de prendre en charge plusieurs clients et vous permet d'interagir avec homecode par programme.

Vous pouvez exécuter `homecode serve` pour démarrer un serveur autonome. Si vous avez le
homecode TUI en cours d'exécution, `homecode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins HomeCode [IDE](/docs/ide).
...
Le serveur homecode expose les API suivantes.
