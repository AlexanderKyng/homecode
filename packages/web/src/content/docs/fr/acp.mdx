---
title: Support ACP
description: Utilisez OpenQCode dans n’importe quel éditeur compatible ACP.
---

OpenQCode prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser OpenQCode via ACP, configurez votre éditeur pour exécuter la commande `openqcode acp`.

La commande démarre OpenQCode en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "OpenQCode": {
      "command": "/absolute/path/bin/openqcode",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « OpenQCode » dans le sélecteur d'agent AI Chat.
...
Pour utiliser OpenQCode comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser OpenQCode comme agent ACP pour le chat.
...
OpenQCode fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration OpenQCode
