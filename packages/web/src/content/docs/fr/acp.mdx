---
title: Support ACP
description: Utilisez HomeCode dans n’importe quel éditeur compatible ACP.
---

HomeCode prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser HomeCode via ACP, configurez votre éditeur pour exécuter la commande `homecode acp`.

La commande démarre HomeCode en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "HomeCode": {
      "command": "/absolute/path/bin/homecode",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « HomeCode » dans le sélecteur d'agent AI Chat.
...
Pour utiliser HomeCode comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser HomeCode comme agent ACP pour le chat.
...
HomeCode fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration HomeCode
