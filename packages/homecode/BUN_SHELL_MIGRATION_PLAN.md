...
Replace runtime Bun shell template-tag usage in `packages/homecode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/homecode/src` is removed except:
