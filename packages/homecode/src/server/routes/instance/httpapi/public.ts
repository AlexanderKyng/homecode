...
import { HomeCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for homecode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(HomeCodeHttpApi)
...
export const PublicApi = HomeCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "homecode",
    version: "1.0.0",
    description: "homecode api",
