...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@homecode-ai/sdk/v2"
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // HomeCode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const homecodeProvider = providers.find((p) => p.id === "homecode")
  if (homecodeProvider) {
    const [best] = Provider.sort(Object.values(homecodeProvider.models))
    if (best) {
      return {
