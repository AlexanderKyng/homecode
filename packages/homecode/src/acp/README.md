...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for homecode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal homecode sessions
...
homecode acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 homecode acp
```
...
- **Terminal Integration**: Full terminal support via homecode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | homecode acp
```
...
ACP sessions map cleanly to homecode's internal session model:
