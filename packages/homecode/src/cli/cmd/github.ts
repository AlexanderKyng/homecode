...
import { ModelsDev } from "@homecode-ai/core/models-dev"
...
const AGENT_USERNAME = "homecode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/homecode.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/homecode-agent"
          const command =
...
jobs:
  homecode:
    if: |
      ...
      startsWith(github.event.comment.body, '/homecode')
    ...
    steps:
      ...
      - name: Run homecode
        uses: anomalyco/homecode/github@latest${envStr}
        with:
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      try {
        ...
        console.log("homecode session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/homecode,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to homecode...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("homecode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `homecode/${type}-${hex}-${timestamp}`
        }
        return `homecode/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/homecode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[homecode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the homecode infrastructure after your response",
