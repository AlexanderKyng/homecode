...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "homecode.mode": input.resolveSession ? "local" : "attach",
      "homecode.initial_input": !!input.initialInput,
      "homecode.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "homecode.directory": ctx.directory,
        "homecode.resume": ctx.resume === true,
        "homecode.agent.name": state.agent,
        "homecode.model.provider": state.model?.providerID,
        "homecode.model.id": state.model?.modelID,
        "homecode.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "homecode.model.provider": model.providerID,
            "homecode.model.id": model.modelID,
            "homecode.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "homecode.model.id": state.model?.modelID,
                "homecode.model.variant": state.activeVariant,
                "homecode.prompt.chars": prompt.text.length,
                "homecode.prompt.parts": prompt.parts.length,
                "homecode.prompt.include_files": includeFiles,
                "homecode.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "homecode.directory": input.directory,
      "homecode.initial_input": !!input.initialInput,
      "homecode.demo": input.demo,
    },
    async () => {
      const sdk = createOpencodeClient({
        baseUrl: "http://homecode.internal",
        fetch: input.fetch,
