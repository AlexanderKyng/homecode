...
import homecode from "./theme/homecode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@homecode-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "homecode")
        draft.active = typeof active === "string" ? active : "homecode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.homecode, store.mode)
    })
