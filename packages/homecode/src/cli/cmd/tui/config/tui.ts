...
import type { DeepMutable } from "@homecode-ai/core/schema"
import type { TuiAttentionSoundName } from "@homecode-ai/plugin/tui"
...
export class Service extends Context.Service<Service, Interface>()("@homecode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old homecode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.homecode`
  ...
  // 4. `.homecode` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".homecode") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".homecode") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "homecode.default",
