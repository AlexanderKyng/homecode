...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from homecode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const homecode = await homecodeFiles(input)
  for (const file of homecode) {
...
async function homecodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "homecode"),
    ...(await Filesystem.findUp(["homecode.json", "homecode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "homecode"))
  }
