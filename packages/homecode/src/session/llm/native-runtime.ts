...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@homecode-ai/llm"
import type { LLMClientShape } from "@homecode-ai/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @homecode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains homecode-owned. The native runtime only adapts
      // the @homecode-ai/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
