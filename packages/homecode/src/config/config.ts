...
import { InstallationLocal, InstallationVersion } from "@homecode-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@homecode-ai/core/schema"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in homecode config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
// Uses the shared `DeepMutable` from `@homecode-ai/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@homecode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["homecode.jsonc", "homecode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "homecode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "homecode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        // Fetch well-known remote configs in parallel instead of sequentially.
        const wellknownEntries = Object.entries(auth).filter(([, value]) => value.type === "wellknown") as [
          string,
          { type: "wellknown"; key: string; token: string },
        ][]

        if (wellknownEntries.length > 0) {
          yield* Effect.forEach(
            ...
            ([key, value]) =>
              Effect.gen(function* () {
                ...
                const wellknownURL = `${url}/.well-known/homecode`
                ...
                if (!remoteConfig.$schema)
                  remoteConfig.$schema =
                    "https://raw.githubusercontent.com/AlexanderKyng/homecode/dev/schema/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("homecode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".homecode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["homecode.json", "homecode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.homecode/plugin(s)` are already local files, so ConfigPlugin.load
