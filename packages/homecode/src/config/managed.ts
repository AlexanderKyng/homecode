...
const MANAGED_PLIST_DOMAIN = "ai.homecode.managed"

// Keys injected by macOS/MDM into the managed plist that are not HomeCode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/homecode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "homecode")
    default:
      return "/etc/homecode"
  }
}
