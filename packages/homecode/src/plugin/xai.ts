...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=homecode` lets xAI attribute homecode-originated
...
<html>
  <head>
    <title>HomeCode - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to HomeCode.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>HomeCode - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire homecode process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `homecode/${InstallationVersion}`)
