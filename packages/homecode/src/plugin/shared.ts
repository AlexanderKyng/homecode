...
export const DEPRECATED_PLUGIN_PACKAGES = ["homecode-openai-codex-auth", "homecode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, homecodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(homecodeVersion) || semver.major(homecodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.homecode
  if (typeof range !== "string") return
  if (!semver.satisfies(homecodeVersion, range)) {
    throw new Error(`Plugin requires homecode ${range} but running ${homecodeVersion}`)
  }
}
