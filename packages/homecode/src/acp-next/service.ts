...
import type { Message, OpencodeClient, SessionMessageResponse } from "@homecode-ai/sdk/v2"
...
export const AuthMethodID = "homecode-login"
...
export class Service extends Context.Service<Service, Interface>()("@homecode/ACPNext/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACPNext.initialize")(function* (params: InitializeRequest) {
    const authMethod: AuthMethod = {
      description: "Run `homecode auth login` in the terminal",
      name: "Login with homecode",
      id: AuthMethodID,
    }
...
async function loadDirectorySnapshot(sdk: OpencodeClient, directory: string) {
  ...
  return Directory.build({
    ...
    providers,
    modes,
    defaultModeID: agents.find((agent) => agent.mode === "primary" && agent.hidden !== true)?.name ?? "plan",
...
async function defaultModelFromSdk(
  ...
  const homecodeProvider = providers[ProviderID.make("homecode")]
  const homecodeModel = homecodeProvider ? Provider.sort(Object.values(homecodeProvider.models))[0] : undefined
  if (homecodeProvider && homecodeModel) return { providerID: homecodeProvider.id, modelID: homecodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPNextError.ServiceFailureError({ safeMessage: "HomeCode service failure", service })
}
