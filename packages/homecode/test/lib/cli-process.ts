// Subprocess test harness for the homecode CLI. Spawns the real binary against
...
//
// Configuration flows through homecode's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for homecode.json
...
//
// Today only `homecode.run` is fully wired. The shape supports adding more
// builders (`homecode.serve(opts)`, `homecode.acp(opts)`, `homecode.auth(...)`)
...
const homecodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(homecodeRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `homecode run`. New flags should
...
// `homecode serve` is a long-lived process — it never exits on its own.
...
// `homecode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `homecode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `homecode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("homecode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("homecode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "homecode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `homecode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("homecode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const homecode: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, homecode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `homecode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "homecode") {
...
// every call site. Use this for any test that needs the homecode CLI fixture.
//
...
// resources (e.g. `homecode.serve`) without an extra `Effect.scoped` wrapper —
