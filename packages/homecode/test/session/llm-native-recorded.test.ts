...
import { HttpRecorder, Redactor } from "@homecode-ai/http-recorder"
...
import { LLMEvent, LLMResponse } from "@homecode-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@homecode-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.homecode.ai/proxy/connections/${connection}/v1`
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["homecode", "native", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["homecode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["homecode", "zen", "native", "tool-loop"],
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the homecode LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "homecode.json"),
      JSON.stringify({ $schema: "https://homecode.ai/config.json", ...scenario.config(model) }),
    ),
  )
