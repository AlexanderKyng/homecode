...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "homecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "homecode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "homecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "homecode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["homecode.json", "homecode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "homecode.json"),
        path.join(tmp.path, "homecode.jsonc"),
        path.join(project, "homecode.json"),
        path.join(project, "homecode.jsonc"),
      ])
    })
  })
