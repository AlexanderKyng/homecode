...
describe("reference", () => {
  it.live("resolves supported local and git config forms", () =>
    Effect.gen(function* () {
      const root = path.resolve("homecode-reference-root")
    ...
    provideTmpdirInstance(
      (_dir) =>
        Effect.gen(function* () {
          ...
          const cache = path.join(Global.Path.repos, "github.com", "homecode-reference-test", "repo")
          ...
          const remoteDir = path.join(remoteRoot, "homecode-reference-test")
        ...
        }),
      {
        config: {
          reference: {
            docs: "homecode-reference-test/repo",
          },
        },
      },
    ),
  ...
  scout.live("refreshes configured git references on new instance init", () =>
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const cache = path.join(Global.Path.repos, "github.com", "homecode-reference-refresh", "repo")
      yield* fs.remove(cache, { recursive: true }).pipe(Effect.ignore)
      yield* Effect.addFinalizer(() => fs.remove(cache, { recursive: true }).pipe(Effect.ignore))

      const source = yield* tmpdirScoped({ git: true })
      const remoteRoot = yield* tmpdirScoped()
      const remoteDir = path.join(remoteRoot, "homecode-reference-refresh")
      const remoteRepo = path.join(remoteDir, "repo.git")
      ...
      yield* githubBase(
        ...
        provideTmpdirInstance(
            ...
            }),
          {
            config: {
              reference: {
                docs: "homecode-reference-refresh/repo",
              },
            },
          },
        ),
