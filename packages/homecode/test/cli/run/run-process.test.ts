// Subprocess integration tests for `homecode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `homecode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OPENCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { cliIt } from "../../lib/cli-process"

describe("homecode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, homecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* homecode.run("say hi")
        homecode.expectExit(result, 0)
  ...
  // (the prompt was accepted, then the upstream provider failed), homecode
  ...
  cliIt.concurrent(
    ...
    ({ llm, homecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* homecode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, homecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* homecode.run("say hi", { format: "json" })
        homecode.expectExit(result, 0)

        const events = homecode.parseJsonEvents(result.stdout)
