...
describe("homecode acp-next (subprocess)", () => {
  cliIt.live(
    "responds to initialize behind OPENCODE_ACP_NEXT",
    ({ homecode }) =>
      Effect.gen(function* () {
        const acp = createAcpClient(yield* homecode.acp({ env: { OPENCODE_ACP_NEXT: "1" } }))
        ...
        expect(initialized.agentInfo?.name).toBe("HomeCode")
        expect(initialized.authMethods?.[0]?.id).toBe("homecode-login")
  ...
  cliIt.live(
    ...
    ({ homecode }) =>
      Effect.gen(function* () {
        ...
        expect(methodId).toBe("homecode-login")
    ...
    ({ homecode }) =>
      Effect.gen(function* () {
        const exitedPromise = yield* Effect.scoped(
          Effect.gen(function* () {
            const acp = yield* homecode.acp({ env: { OPENCODE_ACP_NEXT: "1" } })
            return acp.exited
          }),
        )
  ...
  cliIt.live(
    ...
    ({ homecode }) =>
      Effect.gen(function* () {
        const acp = createAcpClient(yield* homecode.acp())
        const initialized = expectOk(yield* acp.request<InitializeResponse>("initialize", { protocolVersion: 1 }))
