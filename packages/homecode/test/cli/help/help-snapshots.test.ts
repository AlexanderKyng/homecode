...
// terminal sizes. The default homecode tui command is excluded —
// `homecode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `homecode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real homecode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("homecode CLI help-text snapshots", () => {
    ...
    ({ homecode }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* homecode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`homecode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`homecode ${argv.join(" ")} --help`)
        }
