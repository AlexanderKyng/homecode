...
import { Project as ProjectV2 } from "@homecode-ai/core/project"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const homecodeFile = path.join(tmp, ".git", "homecode")
      expect(yield* Effect.promise(() => Bun.file(homecodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "homecode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "homecode")
      const wrongCache = path.join(parentDir, ".git", "homecode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "homecode")
      const cacheB = path.join(bareB, "homecode")
      const wrongCache = path.join(parentDir, ".git", "homecode")
