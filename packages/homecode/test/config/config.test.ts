...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/homecode")) {
...
const schemaConfig = (config: object) => ({ $schema: "https://homecode.ai/config.json", ...config })
...
const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  AppFileSystem.use.writeWithDirs(path.join(managedConfigDir, filename ?? "homecode.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "homecode.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "homecode.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".homecode"), schemaConfig(input.local)) : undefined,
      ].filter(
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* AppFileSystem.use.readFileString(path.join(dir, "homecode.jsonc"))
      expect(content).toContain('"$schema": "https://homecode.ai/config.json"')
    ...
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* AppFileSystem.use.existsSafe(path.join(dir, "homecode.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
      ),
    )
  }),
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://homecode.ai/config.json", shell: "bash" },
      "config.json",
    )
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* AppFileSystem.use.readJson(path.join(dir, "homecode.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "homecode.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "homecode.jsonc")
...
it.instance("ignores legacy tui keys in homecode config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, "homecode.jsonc"),
      `{
        // This is a comment
        "$schema": "https://homecode.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* AppFileSystem.use.writeWithDirs(
        path.join(test.directory, "homecode.json"),
      ...
      const content = yield* AppFileSystem.use.readFileString(path.join(test.directory, "homecode.json"))
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { homecode: { options: { apiKey: "{env:OPENCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["homecode"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(path.join(test.directory, "homecode.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .homecode directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .homecode/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .homecode/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .homecode/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".homecode", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "homecode.jsonc")
...
it.instance("local .homecode config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://homecode.ai/config.json",
    ...
    yield* AppFileSystem.use.ensureDir(path.join(test.directory, ".homecode"))
    yield* writeConfigEffect(
      path.join(test.directory, ".homecode"),
      {
        $schema: "https://homecode.ai/config.json",
        mcp: {
          docs: {
...
remoteProjectOverride.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remoteProjectOverride.seen.wellKnown).toBe("https://example.com/.well-known/homecode")
...
trailingSlashWellKnown.it.instance("wellknown URL with trailing slash is normalized", () =>
  Effect.gen(function* () {
    ...
    expect(trailingSlashWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/homecode")
  }),
)
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/homecode`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/homecode.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(templatedHeaderWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/homecode")
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/homecode.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/homecode.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/homecode.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/homecode.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/homecode.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/homecode.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/homecode.json")
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "homecode.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-homecode@2.4.3", file)).toBe("oh-my-homecode@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-homecode@2.4.3", "file:///project/.homecode/plugin/oh-my-homecode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.homecode/plugin/demo.ts", "file:///project/.homecode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.homecode/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".homecode", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  it.instance("skips project .homecode/ directories when flag is set", () =>
    withProcessEnv(
      "OPENCODE_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".homecode", "command", "test-cmd.md"),
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "HomeCode Managed",
          PayloadIdentifier: "ai.homecode.managed.test",
          PayloadType: "ai.homecode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://homecode.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://homecode.ai/config.json")
})
