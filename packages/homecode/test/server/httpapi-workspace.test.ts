...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-homecode-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://homecode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-homecode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-homecode-workspace")
      } finally {
