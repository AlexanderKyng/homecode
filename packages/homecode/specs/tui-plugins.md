...
- Author package entrypoint is `@homecode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://homecode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/homecode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@homecode-ai/plugin/tui`.
- `@homecode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@homecode-ai/plugin/tui"
...
- The value is a semver range checked against the running HomeCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.homecode` is absent, no check is performed (backward compatible).
...
- `homecode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `homecode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.homecode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.homecode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.homecode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.homecode` dir or `OPENCODE_CONFIG_DIR`, HomeCode installs `@homecode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@homecode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus HomeCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
HomeCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, HomeCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@homecode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"homecode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `homecode.default`.
...
- Local plugins persist installed themes under the local `.homecode/themes` area near the plugin config source.
...
- Local smoke plugin: `.homecode/plugins/tui-smoke.tsx`
- Local vim plugin: `.homecode/plugins/tui-vim.tsx`
- Local smoke config: `.homecode/tui.json`
- Local smoke theme: `.homecode/plugins/smoke-theme.json`
