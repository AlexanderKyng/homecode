...
Delete `packages/homecode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/homecode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/homecode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/homecode`
- TUI plugin loader tests in `packages/homecode` if runtime plugin API wiring changed
