...
Practical reference for a future `packages/server` split after the homecode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/homecode`.
...
Until enough shared service code lives outside `packages/homecode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/homecode`

It should not import `packages/homecode` services while `packages/homecode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on homecode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/homecode` implementation details.
