...
Trim `packages/homecode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/homecode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/homecode`.
- `[x]` `bun typecheck` from `packages/homecode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/homecode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/homecode`.
- `[x]` `bun dev generate > /tmp/homecode-openapi.json` from `packages/homecode`.
...
- Edit `packages/homecode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/homecode-openapi.json` from `packages/homecode`.
- `./packages/sdk/js/script/build.ts` from repo root.
- Inspect generated SDK request param types before deleting each override.
- `bun typecheck` from `packages/homecode`.

### PR 4: Move Path Parameter Patterns Into ID Schemas

- Audit `PathParameterSchemas` and `pathParameterSchema()` in `public.ts`.
- Check source schemas in files like `packages/homecode/src/session/schema.ts`, `packages/homecode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/homecode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/homecode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
