...
import { createOpencode, type ToolPart } from "@openqcode-ai/sdk"
...
console.log("🚀 Starting openqcode server...")
const openqcode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await openqcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openqcode session...")
    const { client, server } = openqcode
    ...
    console.log("✅ Created openqcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to openqcode:", message.text)
