...
import { createOpencode, type ToolPart } from "@homecode-ai/sdk"
...
console.log("🚀 Starting homecode server...")
const homecode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await homecode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new homecode session...")
    const { client, server } = homecode
    ...
    console.log("✅ Created homecode session:", createResult.data.id)
  ...
  console.log("📝 Sending to homecode:", message.text)
