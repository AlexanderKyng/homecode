...
import { OpencodePlugin } from "@openqcode-ai/core/plugin/provider/openqcode"
...
describe("OpencodePlugin", () => {
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* load((catalog) => {
          const item = provider("openqcode")
          catalog.provider.update(item.id, () => {})
          const paid = model("openqcode", "paid", { cost: cost(1) })
          catalog.model.update(item.id, paid.id, (draft) => {
            draft.cost = [...paid.cost]
          })
        })
        expect((yield* catalog.provider.get(ProviderV2.ID.openqcode)).options.aisdk.provider.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.openqcode, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
    ...
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* load((catalog) => {
          const item = provider("openqcode")
          catalog.provider.update(item.id, () => {})
          const free = model("openqcode", "free", { cost: cost(0) })
          catalog.model.update(item.id, free.id, (draft) => {
            draft.cost = [...free.cost]
          })
        })
        expect((yield* catalog.provider.get(ProviderV2.ID.openqcode)).options.aisdk.provider.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.openqcode, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
    ...
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* load((catalog) => {
          const item = provider("openqcode")
          catalog.provider.update(item.id, () => {})
          const outputOnly = model("openqcode", "output-only", { cost: cost(0, 1) })
          catalog.model.update(item.id, outputOnly.id, (draft) => {
            draft.cost = [...outputOnly.cost]
          })
        })
        expect((yield* catalog.provider.get(ProviderV2.ID.openqcode)).options.aisdk.provider.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.openqcode, ModelV2.ID.make("output-only"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("uses OPENCODE_API_KEY as credentials", () =>
    withEnv({ OPENCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.openqcode)).options.aisdk.provider.apiKey).toBeUndefined()
        expect((yield* catalog.model.get(ProviderV2.ID.openqcode, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined, CUSTOM_OPENCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        ...
        const load = yield* catalog.loader()
        yield* load((catalog) => {
          const item = provider("openqcode", { env: ["CUSTOM_OPENCODE_API_KEY"] })
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.openqcode)).options.aisdk.provider.apiKey).toBe("configured")
        expect((yield* catalog.model.get(ProviderV2.ID.openqcode, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("uses auth-enabled providers as credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        const load = yield* catalog.loader()
        yield* load((catalog) => {
          const item = provider("openqcode", { enabled: { via: "account", service: "openqcode" } })
          catalog.provider.update(item.id, (draft) => {
  ...
  it.effect("ignores non-openqcode providers and models", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
  ...
  it.effect("prefers gpt-5-nano as the openqcode small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.openqcode
