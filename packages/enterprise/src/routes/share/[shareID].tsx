import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@openqcode-ai/sdk/v2"
import { SessionTurn } from "@openqcode-ai/ui/session-turn"
import { SessionReview } from "@openqcode-ai/ui/session-review"
import { DataProvider } from "@openqcode-ai/ui/context"
import { FileComponentProvider } from "@openqcode-ai/ui/context/file"
import { WorkerPoolProvider } from "@openqcode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@openqcode-ai/ui/logo"
...
import { MessageNav } from "@openqcode-ai/ui/message-nav"
import { FileSSR } from "@openqcode-ai/ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@openqcode-ai/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/openqcode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | OpenQCode</Title>
              </Show>
              <Meta name="description" content="openqcode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/openqcode"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://openqcode.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
