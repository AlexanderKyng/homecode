...
describe("OpenAI Chat route", () => {
    ...
    LLMClient.generate(
      LLM.updateRequest(request, {
        model: Azure.configure({
          baseURL: "https://homecode-test.openai.azure.com/openai/v1/",
          apiKey: "azure-key",
      ...
      Effect.provide(
        dynamicResponse((input) =>
          Effect.gen(function* () {
            const web = yield* HttpClientRequest.toWeb(input.request).pipe(Effect.orDie)
            expect(web.url).toBe("https://homecode-test.openai.azure.com/openai/v1/chat/completions?api-version=v1")
  ...
  it.effect("serializes assistant reasoning content", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          id: "req_reasoning",
      ...
      expect(prepared.body.messages).toContainEqual({ role: "assistant", content: null, reasoning_content: "hidden" })
    }),
  )
