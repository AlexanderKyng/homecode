  ...
  it.instance("does not ask for external_directory when alias path is allowed", () =>
    Effect.gen(function* () {
      ...
      const tmp = yield* Effect.acquireRelease(
        Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "openqcode-grep-alias-"))),
  ...
  scout.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cache = path.join(Global.Path.repos, "github.com", "openqcode-grep-reference", "repo")
        ...
        const remoteDir = path.join(remoteRoot, "openqcode-grep-reference")
      ...
      }),
    {
      config: {
        reference: {
          docs: "openqcode-grep-reference/repo",
        },
      },
    },
  )
