...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-openqcode-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://openqcode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openqcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openqcode-workspace")
      } finally {
