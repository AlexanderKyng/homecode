...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      // This starts a second HTTP server that stands in for the openqcode server
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          ...
          "x-openqcode-directory": "/secret/path",
          "x-openqcode-workspace": "internal",
        }),
        HttpClient.execute,
      )
      ...
      expect(forwarded?.headers["x-openqcode-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-openqcode-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-openqcode-directory", headerDir),
        HttpClient.execute,
      )
