...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".openqcode", "skills", "project-rest-skill", "SKILL.md"),
      `---
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("openqcode", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("openqcode", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
