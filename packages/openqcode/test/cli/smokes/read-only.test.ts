...
describe("openqcode read-only commands (smoke)", () => {
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const r = yield* openqcode.spawn(["mcp", "list"])
        openqcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const r = yield* openqcode.spawn(["providers", "list"])
        openqcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const r = yield* openqcode.spawn(["models"])
        openqcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const r = yield* openqcode.spawn(["agent", "list"])
        openqcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const r = yield* openqcode.spawn(["session", "list"])
        openqcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const r = yield* openqcode.spawn(["stats"])
        openqcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const r = yield* openqcode.spawn(["db", "path"])
        openqcode.expectExit(r, 0, "db path")
