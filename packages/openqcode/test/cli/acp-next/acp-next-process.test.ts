...
describe("openqcode acp-next (subprocess)", () => {
  cliIt.live(
    "responds to initialize behind OPENCODE_ACP_NEXT",
    ({ openqcode }) =>
      Effect.gen(function* () {
        const acp = createAcpClient(yield* openqcode.acp({ env: { OPENCODE_ACP_NEXT: "1" } }))
        ...
        expect(initialized.agentInfo?.name).toBe("OpenQCode")
        expect(initialized.authMethods?.[0]?.id).toBe("openqcode-login")
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        ...
        expect(methodId).toBe("openqcode-login")
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const exitedPromise = yield* Effect.scoped(
          Effect.gen(function* () {
            const acp = yield* openqcode.acp({ env: { OPENCODE_ACP_NEXT: "1" } })
            return acp.exited
          }),
        )
  ...
  cliIt.live(
    ...
    ({ openqcode }) =>
      Effect.gen(function* () {
        const acp = createAcpClient(yield* openqcode.acp())
        const initialized = expectOk(yield* acp.request<InitializeResponse>("initialize", { protocolVersion: 1 }))
