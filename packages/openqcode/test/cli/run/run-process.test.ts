// Subprocess integration tests for `openqcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `openqcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OPENCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { cliIt } from "../../lib/cli-process"

describe("openqcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, openqcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openqcode.run("say hi")
        openqcode.expectExit(result, 0)
  ...
  // (the prompt was accepted, then the upstream provider failed), openqcode
  ...
  cliIt.concurrent(
    ...
    ({ llm, openqcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openqcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, openqcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openqcode.run("say hi", { format: "json" })
        openqcode.expectExit(result, 0)

        const events = openqcode.parseJsonEvents(result.stdout)
