// Subprocess test harness for the openqcode CLI. Spawns the real binary against
...
//
// Configuration flows through openqcode's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for openqcode.json
...
//
// Today only `openqcode.run` is fully wired. The shape supports adding more
// builders (`openqcode.serve(opts)`, `openqcode.acp(opts)`, `openqcode.auth(...)`)
...
const openqcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(openqcodeRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `openqcode run`. New flags should
...
// `openqcode serve` is a long-lived process — it never exits on its own.
...
// `openqcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `openqcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `openqcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly openqcode: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("openqcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("openqcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "openqcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `openqcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("openqcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const openqcode: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, openqcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `openqcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "openqcode") {
...
// every call site. Use this for any test that needs the openqcode CLI fixture.
//
...
// resources (e.g. `openqcode.serve`) without an extra `Effect.scoped` wrapper —
