...
describe("reference", () => {
  it.live("resolves supported local and git config forms", () =>
    Effect.gen(function* () {
      const root = path.resolve("openqcode-reference-root")
    ...
    provideTmpdirInstance(
      (_dir) =>
        Effect.gen(function* () {
          ...
          const cache = path.join(Global.Path.repos, "github.com", "openqcode-reference-test", "repo")
          ...
          const remoteDir = path.join(remoteRoot, "openqcode-reference-test")
        ...
        }),
      {
        config: {
          reference: {
            docs: "openqcode-reference-test/repo",
          },
        },
      },
    ),
  ...
  scout.live("refreshes configured git references on new instance init", () =>
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const cache = path.join(Global.Path.repos, "github.com", "openqcode-reference-refresh", "repo")
      yield* fs.remove(cache, { recursive: true }).pipe(Effect.ignore)
      yield* Effect.addFinalizer(() => fs.remove(cache, { recursive: true }).pipe(Effect.ignore))

      const source = yield* tmpdirScoped({ git: true })
      const remoteRoot = yield* tmpdirScoped()
      const remoteDir = path.join(remoteRoot, "openqcode-reference-refresh")
      const remoteRepo = path.join(remoteDir, "repo.git")
      ...
      yield* githubBase(
        ...
        provideTmpdirInstance(
            ...
            }),
          {
            config: {
              reference: {
                docs: "openqcode-reference-refresh/repo",
              },
            },
          },
        ),
