    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".openqcode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const openqcodeModel = {
      ...openaiModel,
      providerID: "openqcode",
      api: {
        id: "openqcode-test",
        url: "https://api.openqcode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, openqcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.openqcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.openqcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          openqcode: { itemId: "msg_openqcode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              openqcode: { itemId: "msg_openqcode_part" },
    ...
    expect(result[0].providerOptions?.openqcode?.itemId).toBe("msg_openqcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.openqcode?.itemId).toBe("msg_openqcode_part")
