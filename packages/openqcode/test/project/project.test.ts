...
import { Project as ProjectV2 } from "@openqcode-ai/core/project"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const openqcodeFile = path.join(tmp, ".git", "openqcode")
      expect(yield* Effect.promise(() => Bun.file(openqcodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "openqcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "openqcode")
      const wrongCache = path.join(parentDir, ".git", "openqcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "openqcode")
      const cacheB = path.join(bareB, "openqcode")
      const wrongCache = path.join(parentDir, ".git", "openqcode")
