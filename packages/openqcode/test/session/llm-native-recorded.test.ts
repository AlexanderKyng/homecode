...
import { HttpRecorder, Redactor } from "@openqcode-ai/http-recorder"
...
import { LLMEvent, LLMResponse } from "@openqcode-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@openqcode-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.openqcode.ai/proxy/connections/${connection}/v1`
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["openqcode", "native", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["openqcode", "native", "oauth", "tool-loop"],
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the openqcode LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "openqcode.json"),
      JSON.stringify({ $schema: "https://openqcode.ai/config.json", ...scenario.config(model) }),
    ),
  )
