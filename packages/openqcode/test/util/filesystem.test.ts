...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openqcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openqcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openqcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openqcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openqcode.json", "openqcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "openqcode.json"),
        path.join(tmp.path, "openqcode.jsonc"),
        path.join(project, "openqcode.json"),
        path.join(project, "openqcode.jsonc"),
      ])
    })
  })
