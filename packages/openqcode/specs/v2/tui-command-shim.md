...
Delete `packages/openqcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/openqcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/openqcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/openqcode`
- TUI plugin loader tests in `packages/openqcode` if runtime plugin API wiring changed
