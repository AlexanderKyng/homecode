...
Trim `packages/openqcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/openqcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/openqcode`.
- `[x]` `bun typecheck` from `packages/openqcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/openqcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/openqcode`.
- `[x]` `bun dev generate > /tmp/openqcode-openapi.json` from `packages/openqcode`.
...
- Edit `packages/openqcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/openqcode-openapi.json` from `packages/openqcode`.
- `./packages/sdk/js/script/build.ts` from repo root.
- Inspect generated SDK request param types before deleting each override.
- `bun typecheck` from `packages/openqcode`.

### PR 4: Move Path Parameter Patterns Into ID Schemas

- Audit `PathParameterSchemas` and `pathParameterSchema()` in `public.ts`.
- Check source schemas in files like `packages/openqcode/src/session/schema.ts`, `packages/openqcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/openqcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/openqcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
