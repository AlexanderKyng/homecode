...
Practical reference for a future `packages/server` split after the openqcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/openqcode`.
...
Until enough shared service code lives outside `packages/openqcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/openqcode`

It should not import `packages/openqcode` services while `packages/openqcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on openqcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/openqcode` implementation details.
