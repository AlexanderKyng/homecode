...
- Author package entrypoint is `@openqcode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://openqcode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/openqcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@openqcode-ai/plugin/tui`.
- `@openqcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openqcode-ai/plugin/tui"
...
- The value is a semver range checked against the running OpenQCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.openqcode` is absent, no check is performed (backward compatible).
...
- `openqcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `openqcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.openqcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openqcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openqcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openqcode` dir or `OPENCODE_CONFIG_DIR`, OpenQCode installs `@openqcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openqcode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus OpenQCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
OpenQCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, OpenQCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@openqcode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"openqcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `openqcode.default`.
...
- Local plugins persist installed themes under the local `.openqcode/themes` area near the plugin config source.
...
- Local smoke plugin: `.openqcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.openqcode/plugins/tui-vim.tsx`
- Local smoke config: `.openqcode/tui.json`
- Local smoke theme: `.openqcode/plugins/smoke-theme.json`
