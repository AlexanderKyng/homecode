...
Replace runtime Bun shell template-tag usage in `packages/openqcode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/openqcode/src` is removed except:
