...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@openqcode-ai/llm"
import type { LLMClientShape } from "@openqcode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("openqcode"))
    return { type: "unsupported", reason: "provider is not openai, openqcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @openqcode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains openqcode-owned. The native runtime only adapts
      // the @openqcode-ai/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
