...
import type { Message, OpencodeClient, SessionMessageResponse } from "@openqcode-ai/sdk/v2"
...
export const AuthMethodID = "openqcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@openqcode/ACPNext/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACPNext.initialize")(function* (params: InitializeRequest) {
    const authMethod: AuthMethod = {
      description: "Run `openqcode auth login` in the terminal",
      name: "Login with openqcode",
      id: AuthMethodID,
    }
...
async function defaultModelFromSdk(
  ...
  const openqcodeProvider = providers[ProviderID.make("openqcode")]
  const openqcodeModel = openqcodeProvider ? Provider.sort(Object.values(openqcodeProvider.models))[0] : undefined
  if (openqcodeProvider && openqcodeModel) return { providerID: openqcodeProvider.id, modelID: openqcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPNextError.ServiceFailureError({ safeMessage: "OpenQCode service failure", service })
}
