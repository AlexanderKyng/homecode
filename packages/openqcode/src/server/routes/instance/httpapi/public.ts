...
import { OpenQCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for openqcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OpenQCodeHttpApi)
...
export const PublicApi = OpenQCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "openqcode",
    version: "1.0.0",
    description: "openqcode api",
