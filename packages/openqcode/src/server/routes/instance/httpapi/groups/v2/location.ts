...
import { LocationServiceMap } from "@openqcode-ai/core/location-layer"
import { PluginBoot } from "@openqcode-ai/core/plugin/boot"
...
>()("@openqcode/ExperimentalHttpApiV2Location") {}

function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  const query = new URL(request.url, "http://localhost").searchParams
  return {
    directory: query.get("location[directory]") || request.headers["x-openqcode-directory"] || process.cwd(),
    workspaceID: query.get("location[workspace]") || request.headers["x-openqcode-workspace"],
  }
}
