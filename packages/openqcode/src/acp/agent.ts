...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@openqcode-ai/sdk/v2"
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // OpenQCode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const openqcodeProvider = providers.find((p) => p.id === "openqcode")
  if (openqcodeProvider) {
    const [best] = Provider.sort(Object.values(openqcodeProvider.models))
    if (best) {
      return {
