...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for openqcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal openqcode sessions
...
openqcode acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 openqcode acp
```
...
- **Terminal Integration**: Full terminal support via openqcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | openqcode acp
```
...
### Mapping to OpenQCode

ACP sessions map cleanly to openqcode's internal session model:
