...
const MANAGED_PLIST_DOMAIN = "ai.openqcode.managed"

// Keys injected by macOS/MDM into the managed plist that are not OpenQCode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/openqcode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "openqcode")
    default:
      return "/etc/openqcode"
  }
}
