...
import { NamedError } from "@openqcode-ai/core/util/error"
import { Flag } from "@openqcode-ai/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { InstallationLocal, InstallationVersion } from "@openqcode-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@openqcode-ai/core/schema"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in openqcode config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for openqcode serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://openqcode.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://openqcode.ai/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@openqcode-ai/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@openqcode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["openqcode.jsonc", "openqcode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://openqcode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://openqcode.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openqcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "openqcode.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://openqcode.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/openqcode`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://openqcode.ai/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("openqcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".openqcode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["openqcode.json", "openqcode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.openqcode/plugin(s)` are already local files, so ConfigPlugin.load
