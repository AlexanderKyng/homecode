...
const TUI_SCHEMA_URL = "https://openqcode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from openqcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const openqcode = await openqcodeFiles(input)
  for (const file of openqcode) {
...
async function openqcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "openqcode"),
    ...(await Filesystem.findUp(["openqcode.json", "openqcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "openqcode"))
  }
