...
import openqcode from "./theme/openqcode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@openqcode-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "openqcode")
        draft.active = typeof active === "string" ? active : "openqcode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.openqcode, store.mode)
    })
