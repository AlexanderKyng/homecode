...
import { createOpencodeClient } from "@openqcode-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "openqcode.mode": input.resolveSession ? "local" : "attach",
      "openqcode.initial_input": !!input.initialInput,
      "openqcode.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "openqcode.directory": ctx.directory,
        "openqcode.resume": ctx.resume === true,
        "openqcode.agent.name": state.agent,
        "openqcode.model.provider": state.model?.providerID,
        "openqcode.model.id": state.model?.modelID,
        "openqcode.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "openqcode.model.provider": model.providerID,
            "openqcode.model.id": model.modelID,
            "openqcode.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "openqcode.model.id": state.model?.modelID,
                "openqcode.model.variant": state.activeVariant,
                "openqcode.prompt.chars": prompt.text.length,
                "openqcode.prompt.parts": prompt.parts.length,
                "openqcode.prompt.include_files": includeFiles,
                "openqcode.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "openqcode.directory": input.directory,
      "openqcode.initial_input": !!input.initialInput,
      "openqcode.demo": input.demo,
    },
    async () => {
      const sdk = createOpencodeClient({
        baseUrl: "http://openqcode.internal",
        fetch: input.fetch,
