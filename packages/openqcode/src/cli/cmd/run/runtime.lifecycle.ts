...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "openqcode.agent.name": input.agent,
      "openqcode.directory": input.directory,
      "openqcode.first": input.first,
      "openqcode.model.provider": input.model?.providerID,
      "openqcode.model.id": input.model?.modelID,
      "openqcode.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "openqcode.show_exit": next.showExit,
