...
export const DEPRECATED_PLUGIN_PACKAGES = ["openqcode-openai-codex-auth", "openqcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, openqcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(openqcodeVersion) || semver.major(openqcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.openqcode
  if (typeof range !== "string") return
  if (!semver.satisfies(openqcodeVersion, range)) {
    throw new Error(`Plugin requires openqcode ${range} but running ${openqcodeVersion}`)
  }
}
