...
# 套件管理員
npm i -g openqcode-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install openqcode             # Windows
choco install openqcode             # Windows
brew install anomalyco/tap/openqcode # macOS 與 Linux（推薦，始終保持最新）
brew install openqcode              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S openqcode            # Arch Linux (Stable)
paru -S openqcode-bin               # Arch Linux (Latest from AUR)
mise use -g openqcode               # 任何作業系統
nix run nixpkgs#openqcode           # 或使用 github:anomalyco/openqcode 以取得最新開發分支
```
...
OpenQCode 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/anomalyco/openqcode/releases) 或 [openqcode.ai/download](https://openqcode.ai/download) 下載。
...
OpenQCode 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，OpenQCode 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://openqcode.ai/docs/agents) 的資訊。

### 線上文件

關於如何設定 OpenQCode 的詳細資訊，請參閱我們的 [**官方文件**](https://openqcode.ai/docs)。

### 參與貢獻

如果您有興趣參與 OpenQCode 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 OpenQCode 進行開發

如果您正在開發與 OpenQCode 相關的專案，並在名稱中使用了 "openqcode"（例如 "openqcode-dashboard" 或 "openqcode-mobile"），請在您的 README 中加入聲明，說明該專案並非由 OpenQCode 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=738j8655-cd59-4633-a30a-1124e0096789&qr_code=true) | [X.com](https://x.com/openqcode)
